...
コード側の G-15〜G-18 は **実装済み**。現行ステータスは [FEATURE_REGISTRY.md](FEATURE_REGISTRY.md) を正本とし、履歴・証跡は各 verification doc を参照する。今後の新規スライスは FEATURE_REGISTRY で管理する。`docs/verification/` 配下の proof・提案メモの読み方と **仕様の正本チェーン**は [verification/README.md](verification/README.md) を参照。
