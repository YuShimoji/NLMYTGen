...
| G-19 | 立ち絵 複数体素材 × ゆっくり顔（body_variant / face_map 束ね） | proposed | L2/L3 | 複数の体素材それぞれに、同一系のゆっくり顔パーツを `face_map`（character-scoped 含む）で解決する運用を機械化。IR から体バリアントを選ぶ語彙または **複数 face_map ファイル＋CLI/registry** のどちらかを設計し、`validate-ir` / `apply-production` と矛盾させない。準備正本: [TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md](verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md)。**承認前は `src/` 変更禁止** |
