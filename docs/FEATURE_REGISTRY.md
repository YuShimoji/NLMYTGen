...
| H-02 | Thumbnail strategy v2 (具体数値優先 + pattern rotation) | approved | L2 | C-08 の上位互換候補。`docs/THUMBNAIL_STRATEGY_SPEC.md` v0.1 で specificity-first / banned pattern / rotation policy / output contract を定義済み。`docs/verification/H02-thumbnail-strategy-ai-monitoring-dry-proof.md` により、repo-local dry proof と `specificity_ledger` / `brief_compliance_check` 契約まで固定した。strict GUI rerun proof は未了 |
