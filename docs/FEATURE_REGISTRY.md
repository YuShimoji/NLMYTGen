...
| G-21 | 茶番劇体テンプレ（外部素材 ImageItem + ゆっくり頭 TachieItem 重畳） | proposed | L2/L3 | 配達員・消防員・アイドル等の**外部人物素材**を ImageItem として配置し、その顔位置にゆっくり頭（TachieItem）を重ねる茶番劇運用を機械化する。**G-19 とは別物**（G-19 はゆっくり系の body_variant 束ね、G-21 は外部素材との重畳）。**構想正本**: [TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md §3.2 体テンプレ構想](verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md)。**新規要素**: IR 語彙 `body_template`（carry-forward、`bg` と同型）/ レジストリ `body_map`（ラベル → ImageItem パス + x/y/zoom、`bg_map`+`slot_map` 合成型）/ patch 拡張（overlay ImageItem 挿入 + slot 座標適用の流用）。**アニメーション同期**: 既存 G-20 `group_motion_map` で頭+体の GroupItem を束ねて同期移動。**表情変化**: 既存 face 機能でそのまま。**X-2 先行確認**: `approved` 昇格前に、既存 group_motion + overlay + face の組み合わせで茶番劇テンプレ ymmp をどこまで再現できるか先行実地確認する（runtime-state `next_action` 参照） |
