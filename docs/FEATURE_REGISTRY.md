...
| G-27 | Real Estate DX background skit review cycle | approved | L2/L3 | **G-24 後継の案件固有 review cycle。** `samples/_probe/g24/real_estate_dx_background_skit_blueprint_validate.json` は `status=blocked` のため、cast motion IR / YMM4 creative acceptance / production timing には進めない。G-27 は `scene bible → validator → overlay/card review → proxy or asset decision → revised blueprint → cast motion IR` の順で、各場面を `production template exists` / `accepted proxy` / `cut from plan` に分類する。close gate は validator `passed`、または production 継続しない明示 blocked closeout。正本: [TASK_DEVELOPMENT_CYCLE_SPEC.md](TASK_DEVELOPMENT_CYCLE_SPEC.md)。現 review surface: `samples/_probe/g24/real_estate_dx_overlay_card_review_map.md`。 |
