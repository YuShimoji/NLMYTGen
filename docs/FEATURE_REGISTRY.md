...
| G-12 | timeline native-template measurement | proposed | L3 | `motion` / `transition` / `bg_anim` を YMM4 native template 参照で扱うか、VideoEffects / transition key の直接書き換えで扱うかを実測で確定する packet。成果物は registry contract・dry-run/readback harness・creative judgement と mechanical write の境界定義 |
