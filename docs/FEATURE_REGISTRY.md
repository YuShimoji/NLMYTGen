...
| G-19 | 立ち絵 複数体素材 × ゆっくり顔（body_variant / face_map 束ね） | done | L2/L3 | **スコープ注意（2026-04-15 明示）**: 本 FEATURE は **ゆっくり系立ち絵（TachieItem）内部の body パーツ束ね**。**外部人物素材（配達員・消防員等）の ImageItem にゆっくり頭を重畳する茶番劇運用は G-21** で別機能として扱う。混同しない。 / 複数の体素材それぞれに、同一系のゆっくり顔パーツを `face_map`（character-scoped 含む）で解決する運用を機械化。**方式**: Option A（ディレクトリ + マニフェスト）。IR に `body_id`（carry-forward）を追加し、`--face-map-bundle` CLI フラグでバンドルレジストリを指定。`validate-ir` に `BODY_ID_UNKNOWN` / `BODY_FACE_MAP_MISS` チェック追加。準備正本: [TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md](verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md)。**スコープ**: body_id はシーンレベル（V1）。キャラ別 body_id は別スライス。`--palette` 経路は single-body のまま |
| G-21 | 茶番劇体テンプレ（外部素材 ImageItem + ゆっくり頭 TachieItem 重畳） | proposed (当面不要判定) | L2/L3 | 配達員・消防員・アイドル等の**外部人物素材**を ImageItem として配置し、その顔位置にゆっくり頭（TachieItem）を重ねる茶番劇運用を機械化する。**G-19 とは別物**（G-19 はゆっくり系の body_variant 束ね、G-21 は外部素材との重畳）。**構想正本**: [TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md §3.2 体テンプレ構想](verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md)。**X-2a proof 結果 (2026-04-16)**: [X2A-haitatsuin-dryrun-proof-2026-04-16.md](verification/X2A-haitatsuin-dryrun-proof-2026-04-16.md) — 配達員 ymmp で dry-run 実施、face 15 / transition 3 / motion 2 件が解説役 TachieItem に適用。配達員体+頭 ImageItem は現 apply-production が触らず（想定通り）、**YMM4 手動 GroupItem 化 + 合成 PNG 運用で茶番劇成立**。G-21 body_map 実装は当面不要、運用ルール追加で足りる。**再起票条件**: ①テンプレ 5 件以上蓄積して手動 GroupItem 化が制作 bottleneck ②合成 PNG の組み合わせ爆発 ③user 明示要望。**新規要素（将来再起票時）**: IR 語彙 `body_template` / レジストリ `body_map`（`bg_map`+`slot_map` 合成型）/ patch 拡張（overlay ImageItem 挿入 + slot 座標適用の流用）。 |
...
| G-22 | Dual-rendering scene_presets（立ち絵 TachieItem + 書き出し PNG の両運用） | proposed | L2/L3 | 同一演出を (A) 立ち絵 TachieItem 経路（既存 face/idle_face/slot）と (B) YMM4 で書き出した立ち絵レンダリング PNG の overlay 経路（既存 G-13 done）で実現可能にし、工数削減と整合性・蓄積を両立する。**G-13/G-19/G-21 との違い**: G-13 は単純 overlay、G-19 はゆっくり内部 body 束ね、G-21 は外部素材+ゆっくり頭 重畳、**G-22 は立ち絵を事前レンダリングした単一 PNG の overlay 挿入**。**実装**: 第一候補は **コード変更なし**（既存 `overlay` + `overlay_map` で完結）。将来拡張（registry メタ `source: "tachie_render"` 等）は proof 後に別起票。**構想正本**: [G22-dual-rendering-tachie-and-png-2026-04-16.md](verification/G22-dual-rendering-tachie-and-png-2026-04-16.md)。先行検討: [VISUAL-TOOL-SELECTION-2026-04-16.md](verification/VISUAL-TOOL-SELECTION-2026-04-16.md)。**使い分け**: 主要キャラ本編は (A)、背景キャラ・ワンシーン登場・サムネ素材は (B)。**承認判定トリガー**: 視覚効果 slice Step 5 proof 完了時に (A)/(B) 併用の実績から approved / 当面不要判定を決定。 |
