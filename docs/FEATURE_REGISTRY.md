...
| A-04 | RSS フィード連携（トピック候補取得） | done | L1 | `fetch-topics` サブコマンド。RSS/Atom URL 直接取得 + OPML購読一覧同期 + Inoreader read-only 入力に対応し、タイトル・記事URL・概要・フィード名・カテゴリを NotebookLM 検索クエリ / 台本テーマ候補へ渡せる。`rss-smoke` で sanitized evidence を生成できる。再審査済み (2026-03-30)、OPML同期 v1 実装済み (2026-05-25)、smoke automation 追加 (2026-05-26) |
...
| B-21 | GUI dashboard panel ingest | done | L2/GUI | `build-gui-dashboard-panel` CLI。`production_pilots/.../gui_dashboard_panel/` に read-only static HTML panel、GUI adapter JSON、panel data、DOM/static readback、review checklist、limitations、validation readback を出す。dashboard readiness ingest を1画面で可視化し、sample fixture、real transcript input gate、draft/offline state、true gates、deferred YMM4/thumbnail lanesを明示する |
| B-22 | YMM4 import preview pack | done | L2 | `build-yymm4-import-preview-pack` CLI。`production_pilots/.../ymm4_import_preview_pack/` に draft CSV copy、CSV inventory、cue packet inventory、Writer IR inventory、import readiness summary、source artifact index、Markdown/HTML preview、checklist、limitations、validation readback を出す。sample fixture と draft/offline state を維持し、YMM4 GUI/import/render、production `.ymmp`、rights/legal/public-ready acceptance は閉じる |
| B-23 | Thumbnail visual proof pack | done | L2 | `build-thumbnail-visual-proof-pack` CLI。`production_pilots/.../thumbnail_visual_proof_pack/` に title/text candidates、thumbnail concepts、visual constraints、static SVG layout proof、HTML/Markdown panel、source artifact index、review checklist、limitations、validation readback を出す。外部画像・ロゴ・写真・スクレイプ素材・画像生成 API を使わず、sample fixture、rights、YMM4/render/publication gate を閉じたまま composition/message review surface にする |
| B-24 | Episode factory template registry | done | L2 | `build-episode-factory-template-registry` CLI。`production_pilots/.../episode_factory_template_registry/` に template registry manifest、7種の reusable template JSON、next episode seed sample、init readiness summary、usage/checklist/limitations/readback を出す。現行 pilot を次 episode 初期化に使える local/offline template registry へ蒸留しつつ、sample_fixture_not_real、draft_offline、rights_boundary、public_upload_closed、yymm4_render_closed を維持し、real transcript、YMM4 GUI/import/render、production `.ymmp`、外部 media、publication は閉じる |
...
| G-19 | 立ち絵 複数体素材 × ゆっくり顔（body_variant / face_map 束ね） | done | L2/L3 | **スコープ注意（2026-04-15 明示）**: 本 FEATURE は **ゆっくり系立ち絵（TachieItem）内部の body パーツ束ね**。**外部人物素材（配達員・消防員等）の茶番劇演者**は別系統で扱い、closed G-24 template-first foundation / G-27 review cycle と混同しない。 / 複数の体素材それぞれに、同一系のゆっくり顔パーツを `face_map`（character-scoped 含む）で解決する運用を機械化。**方式**: Option A（ディレクトリ + マニフェスト）。IR に `body_id`（carry-forward）を追加し、`--face-map-bundle` CLI フラグでバンドルレジストリを指定。`validate-ir` に `BODY_ID_UNKNOWN` / `BODY_FACE_MAP_MISS` チェック追加。準備正本: [TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md](verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md)。**スコープ**: body_id はシーンレベル（V1）。キャラ別 body_id は別スライス。`--palette` 経路は single-body のまま |
...
| G-24 | 茶番劇 Group テンプレ生成・解決運用 | done | L2/L3 | **基盤クローズ。** 配達員などの外部素材演者を `speaker_tachie` と分離し、**repo-tracked YMM4 template source → registry 解決 → patch-ymmp GroupItem 自動配置**の流れを正本化した。v1 planned set 5 件は `direct_proven`、`samples/templates/skit_group/delivery_v1_templates.ymmp` に 5/5 同梱済み、`--skit-group-only` で face/bg 未解決を切り離して exact / fallback を自動配置できる。G-24 は template-first skit_group 基盤の成果で閉じる。Real Estate DX 固有の cast/props 不足、proxy 採否、場面ごとの良し悪し判断は G-27 へ切り出す。正本: [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md)。運用サイクル: [TASK_DEVELOPMENT_CYCLE_SPEC.md](TASK_DEVELOPMENT_CYCLE_SPEC.md)。shared registry 雛形: `samples/registry_template/skit_group_registry.template.json`。template source: `samples/templates/skit_group/delivery_v1_templates.ymmp`。 |
| G-27 | Real Estate DX background skit review cycle | hold | L2/L3 | **case-specific evidence retained; active work superseded by G-28.** G-27 は Real Estate DX 固有の review cycle / diagnostic carrier / GUI Review Console / SCS lessons として保持するが、production carrier 未受領 blocker として延命しない。`samples/_probe/g24/real_estate_dx_diagnostic_carrier.ymmp` は diagnostic evidence のまま残し、production carrier へ昇格しない。`samples/_probe/g24/real_estate_dx_review_decisions.json` 不在は G-27 個別 handback の未成立として残すが、汎用 screen carrier 設計の blocker にはしない。G-27 の probe 失敗は [SCENE_COMPOSITION_SCHEMA.md](SCENE_COMPOSITION_SCHEMA.md) の `indexed_whiteboard` / `shape_size_mode_invalid` / `drawing_semantics_calibration` lesson として参照する。 |
| G-28 | Reference-Driven Generic Screen Carrier（参照画像ベース汎用画面 carrier 設計） | proposed | L2/L3 | 人間が渡す参照画像 3-7 枚から、丸コピーではなく構図・密度・余白・色階層・視線誘導・UI感を抽出し、SCS の composition type / visual role / text budget / safe area へ変換する汎用 screen carrier archetype を定義する。出力は reference style brief、SCS mapping、generic carrier archetype、YMM4 item name / group 構成案、個別テーマ variant plan。画像取得、著作権不明画像や reference image URL/raw image/private data の commit、YMM4 `.ymmp` ゼロ生成、render、production timing、creative final acceptance は含めない。初回正本: [REFERENCE_DRIVEN_SCREEN_CARRIER_SPEC.md](REFERENCE_DRIVEN_SCREEN_CARRIER_SPEC.md)。 |
...
| Web UI (現時点) | コアパイプライン優先 | [INVARIANTS.md](INVARIANTS.md) |
| API サーバー (現時点) | コアパイプライン優先 | [INVARIANTS.md](INVARIANTS.md) |
...
| B-26 | Factory seed to content spine dry-run | done | L2 | `build-content-spine-from-seed` CLI. `production_pilots/yukkuri_newsroom_content_spine_002/` consumes `production_pilots/.../factory_seed_dry_run_002/content_spine_input_candidate.json` through the standard content spine builder, then emits `content_spine_dry_run_manifest.json`, `source_seed_reference.json`, `source_artifact_index.json`, `validation_readback.json`, and standard content spine outputs including `MANIFEST.json`, `topic_candidates.json`, `episode_candidate_001.md`, `thumbnail_brief_001.md`, `dashboard_status.json`, and `content_spine_readback.json`. It proves the dry-run seed can feed the first downstream planning stage without manually copying the original pilot, separates inherited defaults / synthetic dry-run placeholders / required real inputs / generated content spine outputs, preserves `dry_run`, `sample_fixture_not_real`, `no_real_transcript`, `rights_boundary`, `public_upload_closed`, and `yymm4_render_closed`, and keeps IR bridge, real transcript, YMM4 GUI/import/render, production `.ymmp`, external media/API/OAuth/payment, and publication closed. |
| B-27 | Content spine 002 IR/CSV bridge dry-run | done | L2 | Existing `build-content-ir-bridge` now emits the second local/offline bridge package at `production_pilots/yukkuri_newsroom_content_spine_002/ir_bridge/`, including `bridge_manifest.json`, `episode_bridge.json`, `writer_ir_candidate.json`, `cue_packet_candidate.{json,md}`, `draft_yymm4.csv`, `ymm4_csv_readiness.md`, `source_content_spine_reference.json`, `source_artifact_index.json`, `review_checklist.md`, `source_to_ir_mapping.md`, `limitations.md`, and `validation_readback.json`. It separates seed-origin fields / inherited template defaults / dry-run placeholders / required real inputs / generated content-spine outputs / generated IR-CSV outputs, preserves `dry_run`, `sample_fixture_not_real`, `no_real_transcript`, `no_yymm4_import`, `rights_boundary`, `public_upload_closed`, and `yymm4_render_closed`, validates headerless draft CSV rows and source indexing, and keeps real transcript, YMM4 GUI/import/render, production `.ymmp`, external media/API/OAuth/payment, rights/public-ready acceptance, and publication closed. |
| B-28 | Content spine 002 transcript substitution readiness | done | L2 | Existing `build-transcript-substitution` now emits the second local/offline transcript readiness package at `production_pilots/yukkuri_newsroom_content_spine_002/transcript_substitution_readiness/`, including `substitution_manifest.json`, `transcript_input_contract.json`, `transcript_source_probe.json`, regenerated episode bridge / Writer IR / cue packet / draft YMM4 CSV, `cue_packet_readiness.json`, `source_context_reference.json`, `source_artifact_index.json`, `review_checklist.md`, `source_to_transcript_mapping.md`, limitations, validation readback, `real_input/README.md`, and a local sample fixture. It consumes the current episode 002 content spine and `ir_bridge/`, separates seed-origin fields / inherited defaults / dry-run placeholders / required real inputs / generated IR-CSV outputs / transcript placeholders / generated transcript outputs, preserves `dry_run`, `sample_fixture_not_real`, `no_real_transcript`, `no_yymm4_import`, `rights_boundary`, `public_upload_closed`, and `yymm4_render_closed`, and keeps real transcript acceptance, YMM4 GUI/import/render, production `.ymmp`, external media/API/OAuth/payment, rights/public-ready acceptance, and publication closed. |
