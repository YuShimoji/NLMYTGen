...
| B-21 | Episode 002 YMM4 import preview pack | done | L2/GUI | `build-yymm4-import-preview-pack` CLI + `src/pipeline/yymm4_import_preview_pack.py`. Generates `production_pilots/yukkuri_newsroom_content_spine_002/ymm4_import_preview_pack/` with import manifest, CSV inventory, cue packet inventory, Writer IR inventory, readiness summary, source artifact index, copied `draft_yymm4_preview.csv`, Markdown panel/checklist, limitations, and validation readback. It records the current CSV as headerless with 9 rows and marks `dry_run`, `sample_fixture_not_real`, `no_real_transcript`, `not_imported_to_yymm4`, `no_yymm4_import`, `no_production_ymmp`, `no_production_thumbnail_acceptance`, and `validation_noise_nonblocking` without launching/importing/rendering in YMM4. It also reads the existing episode 002 thumbnail visual proof as `thumbnail_proof_context` only, so `headline_driven` can be visible as review context without revising thumbnail assets or claiming production thumbnail approval. |

| B-22 | Episode 002 thumbnail visual proof pack | done | L2/GUI | `build-thumbnail-visual-proof-pack` CLI + `src/pipeline/thumbnail_visual_proof_pack.py`. Generates `production_pilots/yukkuri_newsroom_content_spine_002/thumbnail_visual_proof_pack/` with manifest, `thumbnail_variants.json`, 3 deterministic 16:9 SVG variants, `thumbnail_visual_proof.html`, Markdown panel, SVG contact sheet, limitations, source index, and readback. It keeps the work static/local and proof-only, marks `headline_driven` as the recommended review direction, and keeps production thumbnail approval, public readiness, rights acceptance, YouTube upload, external media, YMM4 import/render, and production `.ymmp` gates closed. |
| B-23 | Episode 002 surface alignment pack | done | L2/GUI | `build-surface-alignment-pack` CLI + `src/pipeline/surface_alignment_pack.py`. Generates `production_pilots/yukkuri_newsroom_content_spine_002/surface_alignment_pack/` with manifest, summary, surface status matrix, source artifact crosswalk, boundary consistency report, next-action consistency report, Markdown review story, checklist, limitations, and validation readback. It consumes the existing GUI dashboard panel, YMM4 import preview pack, and thumbnail visual proof pack; records GUI/import/thumbnail statuses as ready; classifies label/next-action drift without rewriting the underlying surfaces; and keeps real transcript, YMM4 import/render, public upload, rights/public-ready, final thumbnail approval, external media, and production `.ymmp` gates closed. |

| B-24 | Episode 002 surface alignment reviewer packet | done | L2/GUI | `build-surface-reviewer-packet` CLI + `src/pipeline/surface_alignment_reviewer_packet.py`. Generates `production_pilots/yukkuri_newsroom_content_spine_002/surface_alignment_review_packet/` with reviewer manifest, aligned review story, repair summary, remaining mismatch ledger, next-action readback, boundary status readback, source crosswalk readback, checklist, limitations, and validation readback. It consumes the accepted surface alignment pack, classifies the prior 8 drift rows as 5 `resolved` and 3 `accepted_nonblocking`, leaves GUI/import/thumbnail source surfaces and creative assets unchanged, and keeps dry-run/sample/no-real-transcript/rights/public/YMM4/render/production-thumbnail/external-media/publication gates closed. |
...
| G-28 | Six-channel Reverse Benchmark Pack（6チャンネル参照逆算パック） | approved | L2/L3 | ユーザー承認済みの後継 scope。実在する6チャンネルから代表動画を一つずつ固定し、channel / video / archetype の一意性、台本・時間軸・画面構成・字幕・立ち絵・素材・motion・音・YMM4適用・最終差分の完全再現 dimension を機械可読化する。quick-win slice は、既存5参照 + 追加1参照を静的 registry / gap matrix / HTML比較面へ束ね、欠落がある対象を `reproduction_ready` にしない validator と CLI build を実装済み。B05 → B02 → B03 → B01 → B04 → B06 の実行順も `execution_queue.json` に固定した。出力は G-02（演出IR）/ G-24（template placement）/ YMM4 operator route の入力契約であり、Python preview/render、`.ymmp` ゼロ生成、素材取得・ダウンロード、public player/YMM4/video の前面起動、音声出力、権利判断、production/publication は含めない。実装正本: `production_pilots/yukkuri_benchmark_six_v1/benchmark_registry.json` + `src/pipeline/yukkuri_benchmark_reproduction.py`。設計背景: [REFERENCE_DRIVEN_SCREEN_CARRIER_SPEC.md](REFERENCE_DRIVEN_SCREEN_CARRIER_SPEC.md)。根拠: USER_REQUEST_LEDGER「6チャンネル・リバースエンジニアリング優先」 + INVARIANTS §UX / Algorithmic Invariants |
