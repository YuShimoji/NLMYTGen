...
| G-12 | timeline native-template measurement | proposed | L3 | `motion` / `transition` / `bg_anim` を YMM4 native template 参照で扱うか、VideoEffects / transition key の直接書き換えで扱うかを実測で確定する packet。readback harness として `measure-timeline-routes` CLI を追加済み。成果物は registry contract・dry-run/readback harness・creative judgement と mechanical write の境界定義 |
| G-13 | overlay / se timeline insertion adapter | proposed | L2/L3 | `overlay` / `se` の意味ラベルを registry 解決し、ymmp へ timing anchor 付きで deterministic に挿入する packet。G-12 で write target が安定した後に進める。目的は場面転換・強調素材の単純な差し込み作業を assistant 側へ寄せつつ、最終的な密度判断と音量判断は人間に残すこと |
...
| H-02 | Thumbnail strategy v2 (具体数値優先 + pattern rotation) | approved | L2 | C-08 の上位互換候補。`docs/THUMBNAIL_STRATEGY_SPEC.md` v0.1 で specificity-first / banned pattern / rotation policy / output contract を定義済み。抽象煽りより、年数・人数・割合・金額・固有名詞など本文根拠のある具体性を優先する |
