...
Packaging Orchestrator brief に `thumbnail_controls.preferred_specifics` がある場合は、
それをどう使ったかを必ず明示してください。

- `preferred_specific_used`: main copy に使った具体項目
- `preferred_specific_not_used`: 使わなかった具体項目と理由
- `strongest_evidence_coverage`: 各 main copy が依拠する本文根拠が opening / body のどこで回収されるか

### Brief Compliance Check

最後に以下を yes/no で示してください。

- `thumbnail_promise_respected`
- `forbidden_overclaim_respected`
- `specificity_first_respected`

`preferred_specifics` があるのに abstract hype を優先した場合は、その理由を短く説明してください。
...
- `preferred_specifics` が与えられている場合、main copy 5案のうち少なくとも 3 案はそのいずれかを明示的に含めてください。満たせない場合は理由を書くこと
```
...
- preferred_specific_used: `71.4%`, `19億ドル`, `吸入器で違反判定`
- preferred_specific_not_used: `9%増`
  - why_not_used: body では強いが、サムネの最短理解では `71.4%` と吸入器エピソードの方が即時性が高い
- strongest_evidence_coverage:
  - `71.4%が秒単位で監視される` → opening後半 / body前半
  - `吸入器で違反` → opening
  - `感謝ボタンは免罪符だった` → body後半

### Brief Compliance Check

- thumbnail_promise_respected: yes
- forbidden_overclaim_respected: yes
- specificity_first_respected: yes
```
