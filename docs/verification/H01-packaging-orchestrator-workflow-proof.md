...
### Sample Packet（P01 Amazon · `p0_phase1_amazon`）
...
| C-07/C-08 入力順の記録 | [h01_lane_d_p01_amazon_c07_input_2026-04-10.md](h01_lane_d_p01_amazon_c07_input_2026-04-10.md) / [h01_lane_d_p01_amazon_c08_input_2026-04-10.md](h01_lane_d_p01_amazon_c08_input_2026-04-10.md) |
...
## Lane D P01 Amazon (`lane_d_p01_amazon_2026-04-10`)

P01 Amazon 台本向けに Packaging Orchestrator brief を新規正本化し、レーン D（トラック D / B-4）の入力順を repo 固定した。

- packaging brief: `samples/packaging_brief_p0_amazon.md`（`video_id`: `p0_phase1_amazon`）
- transcript: `samples/The Amazon Panopticon Surveillance and the Modern Worker.txt`
...
| 観点 | before | after | 判定 |
|---|---|---|---|
| title が台本を侵食していないか | 単体台本のみでは約束が分散しがち | `title_promise` を brief で先に固定 | 改善 |
| thumbnail が本文根拠を離れていないか | 抽象煽りへ流れやすい | `preferred_specifics` / `required_evidence` が台本の数値・逸話と一致 | 改善 |
| abstract hype へ流れていないか | 制約が弱い | `banned_copy_patterns` を毎回先頭制約として添付 | 改善 |
| opening が promise を早めに回収しているか | 取りこぼし検知が曖昧 | 台本先頭に吸入器エピソード・71.4% が存在することを brief と突合 | 適合 |
| C-07 が required_evidence を拾えているか | 視覚回収対象が曖昧化しやすい | brief で must_surface_in を明示 | 改善（実行はオペレータ） |

### Alignment Check（台本本文 vs brief · repo 照合）
...
## 追補: CLI 次サイクル記録用 Operational Run（`p0_nextcycle_amazon_2026-04-10_a`）

P0 次サイクル台本向けに、一時ファイル `samples/packaging_brief_p0_nextcycle_amazon.md` で **brief 全文を台本全文より先に貼る**運用の入力制約確認を行った記録。パッケージング正本は上記 `packaging_brief_p0_amazon.md` に統合済み。

### Strict Comparison Record（`p0_nextcycle_amazon_2026-04-10_a`）

同一台本（`samples/The Amazon Panopticon Surveillance and the Modern Worker.txt`）で、H-01 brief 制約なし/ありの比較観点を nextcycle 案件に適用した記録。

| 観点 | before（brief なし） | after（brief あり） | 判定（yes/no） | メモ |
|---|---|---|---|---|
| promise逸脱 | title/thumbnail/script の約束が分散しやすい | `title_promise` / `thumbnail_promise` を先頭制約で固定 | yes | 当時は `samples/packaging_brief_p0_nextcycle_amazon.md` を先頭投入 |
