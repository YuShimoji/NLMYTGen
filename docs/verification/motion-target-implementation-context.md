...
以下はすべて同一の既定動作 (speaker の TachieItem に適用):
...
| **`["layer:10","layer:11"]`** | **複数レイヤーに同時適用** (2026-04-19 追加) |
| **`[{"layer":10},{"layer":11}]`** | **同上、オブジェクト配列** (2026-04-19 追加) |

#### 今回対象外

- `"group:<remark>"` (GroupItem Remark 指定): canonical template 経路で扱う
- speaker と layer の混在配列 (例: `["speaker","layer:10"]`)
