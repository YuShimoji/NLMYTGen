...
| # | 内容 | 担い手 |
|---|------|--------|
| 1 | `_tmp/b2_haitatsuin_motion_applied*.ymmp` を YMM4 で開き、Layer 10 の配達員 ImageItem に VideoEffects が視覚的に機能するか確認 | user |
| 2 | haitatsuin 配達員の canonical skit_group template を 1 件固定 (YMM4 上で GroupItem + body/head/装飾の基準構成を作成) | user |
| 3 | canonical template から小演出テンプレートを 6-8 件量産 (surprise_jump, panic_shake, deny_shake 等) | user |
| 4 | `skit_group_registry.template.json` を実テンプレートで埋める | user + assistant |
| 5 | 実案件で IR 要求を exact / fallback / manual note で解決し P02 に証跡を残す | user + assistant |
| 6 | `tests/test_ymmp_motion_patch.py` に motion_target ありのユニットテスト追加 | assistant |
