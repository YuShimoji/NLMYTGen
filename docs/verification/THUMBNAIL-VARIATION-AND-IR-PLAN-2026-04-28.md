...
目的: 「YMM4 テンプレを基準に、slot 化された文字・立ち絵・背景を差し替えて書き出す」を基本線にしたまま、毎回違ったサムネイルにするための変奏設計と、台本 / 演出 IR との接続方針を整理する。
...
- 自動化するなら「既存テンプレ内の Text / Image / Group slot を差し替える」まで。最終デザイン判断と YMM4 書き出し確認は人間だが、slot 命名、差し替え候補生成、readback、gap report は assistant/tool 側で先に閉じる。
...
YMM4/native template 側で粗配置する時点で、差し替え対象を `Remark` などで命名しておく。assistant はこの命名候補、slot 表、readback 条件を先に用意する。
...
完全自動レイアウトではなく、operator-approved な YMM4/native template の粗配置を基準にした微調整に留める。
