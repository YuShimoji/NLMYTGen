...
Phase 1 を **1 本の台本**で通すための再現手順。GUI LLM 上の実編集は人間作業のため、ここでは **CLI まで機械再現可能な部分**を正本とする。

## 手順の書き分け（本番方針の二分ではない）

以下は **読みやすさのための二段**であり、**案件ごとにどこまで NotebookLM に近いままにするかは可変**である（現状が原文寄りでも、本番でも常にそうとは限らない）。

| ラベル | 内容 | C-09 |
|--------|------|------|
| **経路 A（機械中心）** | 整形済み／NLM 準拠に近い台本に対し `validate` → `build-csv` → YMM4。必要なら **任意**で `diagnose-script`（B-18）を参照のみ | 使わないことがある |
| **経路 B（C-09 + 反復 refined）** | B-18 の JSON を材料に C-09 で constrained rewrite → refined 稿 → `validate` / `build-csv`。表の **v5〜v14 連鎖**や [P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md) の v14 アンカーは、主に **字幕・overflow 改善のコーパス検証記録**（経路 B 側）。全オペレータの標準 P0 を定義するものではない |

以降の「### 1〜3」は **経路 B を通すときの典型手順**（B-18 → C-09 → build）。**経路 A だけ行う場合**は §「経路 A のみ」の手順へ飛ぶ。

## 舵取り（Phase 1 Block-A 縦優先）

2026-04-11: 主軸案件・Amazon の扱い・YMM4 **作業ブロック（到達ベース）**を [P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md) に固定。

### Block-A と経路 A（再設計クローズ）

- **Block-A**（YMM4 S-4・読込エラーのみ）は [P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md) §2。
- NotebookLM 準拠の台本では **経路 A**（本ファイル §「経路 A のみ」）で機械前置きを完走してよく、**C-09 はこのゲートに含めない**（任意）。
- 手順の正本・コマンド例・P01 表の書き方は [P0-BLOCK-A-AND-PATH-A.md](P0-BLOCK-A-AND-PATH-A.md)。
...
## 経路 A のみ（validate → build-csv → YMM4）

1. 台本パスを決める（例: NotebookLM 由来をほぼそのまま保存した `.txt`）。
2. `uv run python -m src.cli.main validate <台本>`（話者ラベルが入力と一致すること）。
3. `uv run python -m src.cli.main build-csv <台本> -o <出力.csv> ...`（`--speaker-map` / `--unlabeled` 等は入力形式に合わせる。オプションは [PIPELINE_SPEC.md](../PIPELINE_SPEC.md) / CLI `--help`）。
4. YMM4 S-4 で CSV を読み込み、本ファイル末尾の表に **接続判定**を 1 行追記。

（B-18 や C-09 は**必須ではない**。手戻りが多いと判断したら経路 B を検討する。）

## 手順（経路 B 典型: B-18 → C-09 → build）
...
## 字幕: L2 推定と YMM4 実表示のギャップ（任意）

`build-csv --stats` の overflow 候補は **表示幅の推定**であり、YMM4 のフォント・解像度・テンプレと **常に一致しない**。ギャップを記録する手順・1 行メモの型は [VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md](VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md) §4。本表の **メモ**列に短く追記してよい（Phase 1 接続判定の行と同じ表で可）。
...
| 2026-04-10 (`p0_nextcycle_amazon_2026-04-10_a`) | samples/The Amazon Panopticon Surveillance and the Modern Worker.txt | 未（C-09 未実施） | PASS（CLI） | 次サイクル P0 の本開発記録。`diagnose-script --unlabeled --format json`=`samples/p0_nextcycle_amazon_diag.json`、`validate --unlabeled`=`samples/p0_nextcycle_amazon_validate.txt`、`build-csv --unlabeled --max-lines 2 --chars-per-line 40 --reflow-v2 --stats`=`samples/p0_nextcycle_amazon_build.txt` / `samples/p0_nextcycle_amazon_ymm4.csv`（232 rows, overflow 候補 18 行）。YMM4 読込・C-09 介在はオペレータ追記待ち。 |
| 2026-04-10 (`lane_a_amazon_2026-04-10_b`) | samples/The Amazon Panopticon Surveillance and the Modern Worker.txt | 実投入（`samples/amazon_panopticon_lane_a_refined.txt`、C-09 相当の ASR 修正） | PASS（CLI+refined）／YMM4 未 | ファイル4レーンA（Amazon）。`diagnose-script --unlabeled`=`samples/lane_a_amazon_2026-04-10_refined_diag.json`（info: `ROLE_SKIPPED_NO_MAP`）、`validate --unlabeled`=`samples/lane_a_amazon_2026-04-10_refined_validate.txt`（87 utterances）、`build-csv --unlabeled`=`samples/lane_a_amazon_2026-04-10_refined_build_utf8.txt` / `samples/amazon_panopticon_B11_ymm4.csv`（244 rows, overflow 候補 14 行。`p0_nextcycle_amazon_2026-04-10_a` 比で候補純減）。B-11: [B11-workflow-proof-amazon-panopticon-2026-04-10.md](B11-workflow-proof-amazon-panopticon-2026-04-10.md)（§2 取込後は YMM4 実測待ち）。 |
| 2026-04-11 (`lane_a_amazon_2026-04-11_c`) | samples/The Amazon Panopticon Surveillance and the Modern Worker.txt | 実投入（`samples/amazon_panopticon_lane_a_refined.txt`） | PASS（CLI+refined 再検証）／YMM4 未 | ファイル4レーンA（P0）機械再実行。`diagnose-script --unlabeled --format json`=`samples/lane_a_amazon_2026-04-11_refined_diag.json`（info: `ROLE_SKIPPED_NO_MAP` のみ）、`validate --unlabeled`=`samples/lane_a_amazon_2026-04-11_refined_validate.txt`（87 utterances）、`build-csv --unlabeled`=`samples/lane_a_amazon_2026-04-11_refined_build.txt` → `samples/amazon_panopticon_B11_ymm4.csv`（244 rows、overflow 候補 14 行・前回 `lane_a_amazon_2026-04-10_b` と一致）。§2 取込後・4 区分の実測はオペレータ（YMM4 S-4 通し）待ち。 |
| 2026-04-11 (`p0_mainline_v14_steering_2026-04-11_a`) | samples/v14_timing_label_split_refined.txt | 実投入（v14・本編 P0 縦主軸） | PASS（CLI 舵取り再スモーク）／PASS（YMM4 S-4） | 舵取りプラン実装。主軸＝AI監視 v14、Amazon は横（[P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md)）。`diagnose-script`=`samples/p0_steering_v14_2026-04-11_diag.json`（`--speaker-map` `ホストA=れいむ,ホストB=まりさ`、warning 0 件）、`validate`=`samples/p0_steering_v14_2026-04-11_validate.txt`（163 utterances）、`build-csv` ログ=`samples/p0_steering_v14_2026-04-11_build.txt` → `samples/v14_t3_ymm4.csv`（365 rows、[WARN] 16 行・v14 既報と一致）。**タスク再設計**・経路 A との対応は [P0-BLOCK-A-AND-PATH-A.md](P0-BLOCK-A-AND-PATH-A.md)。**2026-04-12**: YMM4 台本読込（S-4）のみ実施、読込エラーなし、字幕の見え方を目視確認（オペレータ申告を正本に反映）。 |
| 2026-04-12 (`p0_block_a_task_redesign_2026-04-12_a`) | samples/v14_timing_label_split_refined.txt | 実投入（v14・機械のみ再確認） | PASS（CLI）／YMM4 は `p0_mainline_v14_steering_2026-04-11_a` 行へ集約 | Block-A タスク再設計クローズ（上記リンク）。`diagnose-script`=`samples/p0_steering_v14_2026-04-12_diag.json`（warning 0 件）、`validate`=`samples/p0_steering_v14_2026-04-12_validate.txt`、`build-csv` ログ=`samples/p0_steering_v14_2026-04-12_build.txt` → `samples/v14_t3_ymm4.csv`（365 rows、[WARN] 16 行）。経路 A 型では C-09 を挟まず機械前置き可。 |
| 2026-04-09 (`lane_a_phase1_2026-04-09_a`) | samples/AI監視が追い詰める生身の労働.txt | 作成済み（`samples/lane_a_2026-04-09_refined.txt`） | PASS（CLI+YMM4 S-4 読込） | レーンA実行。`diagnose-script`=`samples/lane_a_2026-04-09_diag.json`（warning 2件: `EXPLAINER_ROLE_MISMATCH` / `LISTENER_ROLE_MISMATCH`）、`validate`=`samples/lane_a_2026-04-09_validate.txt`（28 utterances parsed）、`build-csv`=`samples/lane_a_2026-04-09_ymm4.csv` / `samples/lane_a_2026-04-09_refined_ymm4.csv`（111 rows, overflow 候補 2 行: row 22/64）。YMM4 S-4 は `samples/lane_a_2026-04-09_refined_ymm4.csv` の読込成立まで確認。話者割り当ては読込UIで手動設定（S-4の通常手順）。背景/overlay/se 等の表示確認は本レーンのスコープ外。 |
...
| 2026-04-12 (`gapreflow_2026-04-12_a`) | samples/AI監視が追い詰める生身の労働.txt | 未（B11 と同一の生台本経路） | PASS（CLI） | ギャップ計測: `chars-per-line=40`、機械 WARN **row22/まりさ**・**row64/れいむ**／YMM4 実機は B11 §2.2（WARN 行も手動改行・辞書 **0**）。ログ `samples/gapreflow_2026-04-12_session_build.txt` |
| 2026-04-13 (`gapreflow_2026-04-13_b`) | samples/v14_timing_label_split_refined.txt | 実投入（v14・P0 主軸と同一 refined） | PASS（CLI）／YMM4 は `p0_mainline_v14_steering_2026-04-11_a` 行へ集約 | ギャップ計測（第2コーパス）: 同一 `build-csv` オプションで機械 WARN **16** 件。CSV `samples/gapreflow_2026-04-13_v14_ymm4.csv`、ログ `samples/gapreflow_2026-04-13_session_build.txt`。詳細表は [VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md](VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md) §4.1.2。 |
| （運用者が追記） | | | | |
