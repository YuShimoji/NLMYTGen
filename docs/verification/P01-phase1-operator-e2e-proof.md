...
2026-04-11: 主軸案件・Amazon の扱い・YMM4 カレンダーブロックを [P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md) に固定。
...
| 2026-04-11 (`p0_mainline_v14_steering_2026-04-11_a`) | samples/v14_timing_label_split_refined.txt | 実投入（v14・本編 P0 縦主軸） | PASS（CLI 舵取り再スモーク）／YMM4 次ブロック | 舵取りプラン実装。主軸＝AI監視 v14、Amazon は横（[P0-VERTICAL-STEERING-2026-04-11.md](P0-VERTICAL-STEERING-2026-04-11.md)）。`diagnose-script`=`samples/p0_steering_v14_2026-04-11_diag.json`（`--speaker-map` `ホストA=れいむ,ホストB=まりさ`、warning 0 件）、`validate`=`samples/p0_steering_v14_2026-04-11_validate.txt`（163 utterances）、`build-csv` ログ=`samples/p0_steering_v14_2026-04-11_build.txt` → `samples/v14_t3_ymm4.csv`（365 rows、[WARN] 16 行・v14 既報と一致）。YMM4 S-4 は **Block-A**（同ファイルのカレンダー表）で実施し本行を更新すること。 |
