...
- GroupItem は「制御ノード」として **中央基準**でテンプレ登録する。
- 配下（頭・体・装飾）は **相対配置**で保存し、実行時は GroupItem の X/Y/Zoom を変更する。
- 立ち絵を任意位置に置いた後で都度 Group 化する使い捨て運用は、標準運用として採用しない。

### 固定理由

- 拡大縮小の基準点が Group 中心になるため、中央基準を崩すとテンプレ間の再現性が落ちる。
- 中央基準テンプレに統一すると、A案（既存 GroupItem 操作）で汎用パッチが設計しやすい。
