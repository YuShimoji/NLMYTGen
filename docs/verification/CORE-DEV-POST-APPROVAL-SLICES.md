...
| T1-RUNBOOK-GUI | 2026-04-11 | 同ドラフト §2.2: [OPERATOR_PARALLEL_WORK_RUNBOOK.md](../OPERATOR_PARALLEL_WORK_RUNBOOK.md) と GUI 導線（[gui-llm-setup-guide.md](../gui-llm-setup-guide.md) 等）の **文言・手順の明確化**のみ。新タブ・F-01/F-02 復活は禁止。検証正本: [T1-RUNBOOK-GUI-terminology-alignment-proof.md](T1-RUNBOOK-GUI-terminology-alignment-proof.md)。 | `72f9ff2`（`docs: T1-RUNBOOK-GUI（runbook↔gui 用語・導線の明確化）`） |
...
2. [ ] 起票対象はドラフト本文の **PASS反映済み入力のみ**を根拠とし、`NEEDS_FIX` / `継続` 入力は §5.5 差し戻し扱いで起票しない。
3. [ ] スライスは **縦スライス1本**（`1スライス=1起票`）で切り、承認後は本ファイル §1 の承認済み表に追加してから実装へ進む。
4. [ ] [FEATURE_REGISTRY.md](../FEATURE_REGISTRY.md) を更新（`proposed` → `approved` → 実装後 `done` の遷移を守る）。
5. [ ] 実装ブランチは `**master` 起点のトピックブランチ**を使用。
6. [ ] `NLMYTGEN_PYTEST_FULL=1 uv run pytest` が緑（フル回帰必須）。
7. [ ] [runtime-state.md](../runtime-state.md) の「最終検証」日付・必要なら `last_change_relation` を更新。
8. [ ] Gate 別の参照:
...
- 2026-04-10: **T1 文書先行クローズ（導線整合）**。`T1-RUNBOOK-GUI-terminology-alignment-proof.md` を起点に、索引 [CORE-DEV-POST-DELEGATION-INDEX.md](CORE-DEV-POST-DELEGATION-INDEX.md) へ `T1-P2-DOCSAMPLE` / `T1-RUNBOOK-GUI` の対称導線を追加。[T1-P2-DOCSAMPLE-p2-staged-rollout-mechanical-proof.md](T1-P2-DOCSAMPLE-p2-staged-rollout-mechanical-proof.md) に runbook / GUI 逆参照節を追記。**実装変更なし**。
- 2026-04-10: T1 境界確認として、H-01 テンプレ差分のバグ修正判定を [T1-H01-packaging-template-bugfix-gate-proof-2026-04-10.md](T1-H01-packaging-template-bugfix-gate-proof-2026-04-10.md) に記録（未承認 FEATURE 追加なし）。
- 2026-04-10: §2 に「ドラフトPASS入力のみ起票」「NEEDS_FIX/継続は起票対象外」「1スライス=1起票」の起票ゲートを追加。
