...
| BN-05 placement contract/proof | `lanes/sports_news/scripts/build_baseball_yymm4_placement_proof.js`, `samples/_probe/baseball/placement/` | Contract and minimal YMM4 transport proof passed mechanical readback. | Manual preview screenshot and PASS/FIX are still required before placement acceptance. |
...
| --- | --- | --- | --- |
| BN-05 manual YMM4 preview is missing | Mechanical `.ymmp` readback cannot prove visible crop, text readability, subtitle collision, or operator acceptance. | `lanes/sports_news/docs/baseball_manual_preview_hands_on_2026-05-26.md` and `samples/_probe/baseball/placement/`. | Mark placement `PASS`, or tune the placement contract once from a concrete `FIX` note. |
