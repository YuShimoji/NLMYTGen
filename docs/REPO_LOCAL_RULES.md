...
- overlay/card-only compact review を user review に渡す場合、HTML / JSON だけで妥当性判断を求めない。必ず reviewer-facing list/map を添え、各場面ごとに `what to show` / `granularity choice` / `why this instead of alternatives` / `reviewer check` を表で示す。JSON/readback は機械 proof、HTML は preview surface であり、manager-facing review report の代替ではない。別案比較が当て馬に見える場合は未完扱いにして、根拠・粒度・一本化理由を先に補う。
