...
- 茶番劇 G-24 を「user-only の配置作業」と表現しない。既存 sample / GroupItem / layer / target labeling が揃う案件では、assistant が rough placement・effect 適用・registry 下準備を先行し、user は YMM4 上の意図確認と canonical template authoring に集中できるようにする。
...
- 2026-04-18: **skit_group 配下アイテム種別を `ImageItem` のみに固定 (構造的明確化)**。user 指摘「2 つの重なった画像を適切に流用するのが目的で、頭の部分は 1 枚画像で十分。連番アニメ立ち絵を毎回設定するコストが過大なため採用しない決定は既に確定済み」を受け、docs 全体で `TachieItem` = 解説役専用 / `skit_group` 配下 = `ImageItem` (body + 顔 + 装飾) の重ね合わせ、という分離を徹底。顔の感情差し替えは `ImageItem.Source` パス置換で吸収し、`TachieFaceParameter` は外部演者には使わない。修正 docs: [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md) §1/§3.2/§8.2 / [INVARIANTS.md](INVARIANTS.md) Responsibility Boundaries / [AUTOMATION_BOUNDARY.md](AUTOMATION_BOUNDARY.md) 設計原則 / [STEP3_YMM4_TEMPLATE_CHECKLIST.md](STEP3_YMM4_TEMPLATE_CHECKLIST.md) 前提 / [samples/registry_template/skit_group_registry.template.json](../samples/registry_template/skit_group_registry.template.json) に `child_item_rule` と `child_composition` 明記。これ以降、実装・手順・registry は全て `ImageItem` 重ね合わせ前提で書く。
- 2026-04-19: **自動化摩擦緩和 3 点適用** (Value Validation Gate スコープ限定 / Commit and push hygiene 分離 / Read-Only Gate 発火条件を user 明示宣言に限定)。修正 docs: [DECISION_GATES.md](ai/DECISION_GATES.md) §Value Validation Gate / §Read-Only Refresh Gate / [WORKFLOWS_AND_PHASES.md](ai/WORKFLOWS_AND_PHASES.md) §Refresh/Reanchor/Scan / §Commit and push hygiene / [CORE_RULESET.md](ai/CORE_RULESET.md) §Read-only audit phases。根拠: 本セッション agent 監査 + user 承認 2026-04-19。
- 2026-04-19: **意思決定注記 (根拠注記) 仕様を INVARIANTS §運用ルールに正本化**。形式 `根拠: <源1> [+ <源2>]`、対象は新方針採用 / 方針撤回 / 主軸・slice・next_action 変更 / FEATURE status 遷移 / 台帳新規追記、対象外は文言微修正 / typo / 内部 refactor。禁止源として verifiable でない抽象文言を明記。根拠: user 指示 2026-04-19 + [INVARIANTS.md](INVARIANTS.md) §運用ルール。
- 2026-04-19: **`tachie_motion_map_library.json` を Phase 2 v2 に更新 (flat + animation params 両方埋め)**。`samples/EffectsSamples_2026-04-15.ymmp` の実サンプル値を基準に各 motion の振幅・周期を設定。panic_shake から RandomZoomEffect を除去 (「体のみ伸び縮み」の原因)。user 視覚確認結果: 「演者全体の所作 proof には body+顔同期が必要」。次ブロックで `motion_target` の複数 layer 配列対応を追加予定。修正 docs: [MOTION_PRESET_LIBRARY_SPEC.md](MOTION_PRESET_LIBRARY_SPEC.md) §4-a/§5-a。生成物 [samples/_probe/b2/build_library_v2.py](../samples/_probe/b2/build_library_v2.py) + v1 バックアップ。根拠: user 指示 2026-04-19 + `b2_haitatsuin_motion_regen_2026-04-19` proof
- 2026-04-20: **`skit_01` の欠落 ManualSample は repo sync / handoff failure として扱う**。`_tmp/skit_ManualSample_01.ymmp` の再作成依頼は禁止。repo 側に残る artifact だけで復旧すること。`session 105 / 107 / 109 / 110`、`sections-nav.spec.js`、`ui-mode-consistency.spec.js`、`gadgets-documents-tree.js` は NLMYTGen と無関係として quarantine 扱い。根拠: user 指示 2026-04-20 + [skit_01-workflow-breakage-audit-2026-04-20.md](verification/skit_01-workflow-breakage-audit-2026-04-20.md)
- 2026-04-20: **ManualSample なしでも判断できる Capability Atlas を repo 正本として整備する**。画面演出は『何が可能か / どの route が正規か / どの effect は probe-only か』を catalog 式に整理し、IR との接合点を 1 枚で判断できるようにする。機械台帳は `python scripts/build_capability_atlas.py` -> `samples/_generated/capability_atlas.json`。根拠: user 指示 2026-04-20 + [TIMELINE_EFFECT_CAPABILITY_ATLAS.md](TIMELINE_EFFECT_CAPABILITY_ATLAS.md)
- 2026-04-20: **`samples/canonical.ymmp` を canonical anchor artifact として採用**。`haitatsuin_delivery_main` / Layer 9 / ImageItem-only / 左向き基準姿勢を repo 正本へ昇格し、`audit-skit-group` success を canonical proof として扱う。一方で次 frontier は ManualSample 再作成ではなく、canonical から派生 native template asset を 1-2 件ずつ保存すること。根拠: user 追加 artifact 2026-04-20 + [G24-canonical-anchor-adoption-2026-04-20.md](verification/G24-canonical-anchor-adoption-2026-04-20.md)
