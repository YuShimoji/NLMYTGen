...
- 2026-04-19: **`tachie_motion_map_library.json` を Phase 2 v2 に更新し、speaker_tachie 専用制限を解除**。user 指摘「ある値だけ変えて他が全てそのままでは動かない。要素は揃っているはずなので丁寧に設定してほしい」を受け、v1 (flat only) から v2 (flat + animation parameters 両方埋め) へ改訂。`samples/EffectsSamples_2026-04-15.ymmp` の実サンプル値を基準に、各 motion の感情表現に応じて振幅・周期を調整。`panic_shake` から `RandomZoomEffect` を除去 (「体のみ伸び縮み」の原因)。汎用台帳として `TachieItem` / `ImageItem` / `GroupItem` 横断で利用可能に位置づけ直す。修正 docs: [MOTION_PRESET_LIBRARY_SPEC.md](MOTION_PRESET_LIBRARY_SPEC.md) §4-a/§5-a/§適用対象 / [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md) §7 G-23 / [INVARIANTS.md](INVARIANTS.md) Responsibility Boundaries。生成スクリプト [samples/_probe/b2/build_library_v2.py](../samples/_probe/b2/build_library_v2.py)、v1 バックアップ [samples/_probe/b2/tachie_motion_map_library_v1_backup.json](../samples/_probe/b2/tachie_motion_map_library_v1_backup.json)。proof `b2_haitatsuin_motion_regen_v3_2026-04-19`。要 user 確認項目: (1) `thinking_zoom` / `focus_zoom` の Zoom 変化パターン (脈動 vs 静止) / (2) `sad_droop` の下方向動作 (戻る vs 戻らない) / (3) 全体の強度スケール (normal / light / heavy)。根拠: user 指示 2026-04-19 + b2_haitatsuin_motion_regen_2026-04-19 proof。
