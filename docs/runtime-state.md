...
- **cautious gate all-pass（2026-04-21）**: standalone export の順番は **manual acceptance → 1 件の production adoption proof → export** のまま固定し、その 3 段すべてが PASS。user が `samples/haitatsuin_2026-04-12_g24_proof.ymmp` を用意し、`samples/_probe/skit_01/skit_01_ir.json` に対する `audit-skit-group` は `exact=5 / fallback=0 / manual_note=0`、`apply-production --dry-run` は `--face-map samples/face_map_bundles/haitatsuin.json --bg-map samples/_probe/b2/palette_extract/bg_map.json` 付きで `success=true` / `group_motion_changes=0` を確認。warning は `bg label 'studio_blue' not found in bg_map` の 1 件のみ。さらに user report により starter 2 件の standalone export も完了したため、Capability Atlas は `enter_from_left` / `surprise_oneshot` のみ `direct_proven` へ昇格できる状態になった。
...
- **workflow breakage（2026-04-20 正本）**: `_tmp/skit_ManualSample_01.ymmp` は repo 内に存在しない。さらに `skit_01` proof docs / prompt は `_tmp/skit_01_v2.ymmp` を指しているが、repo に残っている検証用 ymmp は `_tmp/skit_01_v2_verify.ymmp`。**新しい ManualSample 作成依頼は禁止**。比較・切り分けは [skit_01-workflow-breakage-audit-2026-04-20.md](verification/skit_01-workflow-breakage-audit-2026-04-20.md) + `python3 samples/_probe/skit_01/audit_skit_01_proof.py` で repo 内 artifact のみを使う。
...
- slice: **G-24 茶番劇 Group template-first 運用 — cautious starter-to-proof gate を starter 2 件で完走**。G-23 motion_target ルーティング実装完了 (コミット `1b45ff2`)。G-24 仕様・3 層分離・テスト 358 件全 pass。**assistant 側は starter batch（`enter_from_left` / `surprise_oneshot`）の project-resident copy 前提で registry / preflight / P02 / handoff を同期済みで、manual acceptance・1 件の production adoption proof・standalone native template export も完了した。残る主 frontier は `nod` / `deny_oneshot` / `exit_left` をどの順で同じ gate に乗せるか**。
- next_action: **assistant 先行 + user 先行の 2 系統で進める**。G-24 を user-only と読まない。
  - **assistant 先行 (A)**: 既存の haitatsuin sample / `motion_target` / `group_motion` 経路を使って、rough placement・effect 適用・registry 下準備を進める。`skit_group_registry` は **CLI preflight 入力としては消費される**が patch-time template resolver ではないため、**template 解決の主経路そのもの**ではなく **既存 GroupItem / ImageItem への当たり適用と fail-fast 監査**として扱う
  - **assistant 先行 (B)**: `skit_01` は **CLI preflight + Capability Atlas** で進める。route 選択は [TIMELINE_EFFECT_CAPABILITY_ATLAS.md](TIMELINE_EFFECT_CAPABILITY_ATLAS.md)、template-first の成立可否は `python -m src.cli.main audit-skit-group <ymmp> <ir.json> --skit-group-registry <json>` を正規入口とし、canonical anchor / exact / fallback / manual_note の成立可否を repo 内 artifact だけで判定する。`samples/canonical.ymmp` は canonical anchor proof として使い、old corpus は mechanical motion proof のまま扱う。欠落した ManualSample は前提にしない
  ...
  - **user 先行 (D)**: starter 2 件の export は完了。次の user-owned 作業は、G-24 をさらに広げるなら `delivery_nod_v1` を次の拡張候補として authoring/export するか、現状の 2 件 starter で運用を継続するかの判断
  - **shared 後続 (E)**: `samples/haitatsuin_2026-04-12_g24_proof.ymmp` を cautious proof corpus として維持しつつ、Atlas / registry / handoff は `enter_from_left` / `surprise_oneshot` のみ `direct_proven`、`nod` / `deny_oneshot` / `exit_left` は `template_catalog_only` のまま運用する。次に拡張するなら `nod -> deny_oneshot -> exit_left` の順で同じ gate を繰り返す
...
| **メンテ (並行低負荷)** | ① **台本品質の継続観測 (B-18)** — Block-A 通過済、新台本は [P01-phase1-operator-e2e-proof.md](verification/P01-phase1-operator-e2e-proof.md) に 1 行追記のみ。委任 Prompt 正本 [B18](prompts/B18-script-diagnostics-observation-prompt.md)。② **Packaging brief / one-sheet (H-01/H-02)** — 新しい packaging brief が必要な案件でのみ起動。[PACKAGING_ORCHESTRATOR_SPEC.md](PACKAGING_ORCHESTRATOR_SPEC.md)、[THUMBNAIL_ONE_SHEET_WORKFLOW.md](THUMBNAIL_ONE_SHEET_WORKFLOW.md)、委任 Prompt 正本 [H01](prompts/H01-packaging-brief-prompt.md) / [H02](prompts/H02-thumbnail-one-sheet-prompt.md)。③ **字幕 B-17 残差観測** — drift が見えた時だけ起動。委任 Prompt 正本 [B17](prompts/B17-reflow-residue-observation-prompt.md) | オペレータ (GUI) / 別セッション assistant | 既定順は **B-18 → H-01/H-02 on demand → B-17 on drift**。verification 更新のみで回す |
...
- last_change_relation: **2026-04-21** starter 2 件の standalone export を同期。user report に基づき `delivery_enter_from_left_v1` / `delivery_surprise_oneshot_v1` を **名前そのままの GroupItem template + ImageItem 2 点込み**で standalone native template library に登録済みと確認した。これにより starter 2 件は manual acceptance / 1 件の production adoption proof / standalone export の 3 段を完了。assistant 側は registry / P02 / runtime / handoff / Capability Atlas をこの state に同期し、`skit_group.intent.enter_from_left` と `skit_group.intent.surprise_oneshot` を `direct_proven` へ昇格、`delivery_nod_v1` / `delivery_deny_oneshot_v1` / `delivery_exit_left_v1` は `template_catalog_only` のまま据え置いた。次の拡張順は `nod` → `deny_oneshot` → `exit_left`。
...
- last_verification: **cautious starter-to-proof gate の repo-local 検証と export sync を完走**。repo-local inspection により `samples/canonical.ymmp` で frame 0 `haitatsuin_delivery_main` / frame 306 `delivery_enter_from_left_v1` / frame 658 `delivery_surprise_oneshot_v1` の 3 GroupItem が Layer 9 / `GroupRange=2` / 隣接 Layer 10-11 `ImageItem` ペアで並ぶことを確認。`delivery_enter_from_left_v1` については `InOutMoveFromOutsideFrameEffect` が `IsInEffect=True` / `IsOutEffect=False` であることも確認した。user report では loop / body-face drift なし、`enter_from_left` 側の意図しない退場設定は YMM4 上でカット済み。続けて `python3 -m src.cli.main audit-skit-group samples/haitatsuin_2026-04-12_g24_proof.ymmp samples/_probe/skit_01/skit_01_ir.json --skit-group-registry samples/registry_template/skit_group_registry.template.json --format text` で `exact=5 / fallback=0 / manual_note=0` を確認し、starter 対象の `enter_from_left` / `surprise_oneshot` が production proof corpus 上でも `exact` であることを再確認。さらに `python3 -m src.cli.main apply-production samples/haitatsuin_2026-04-12_g24_proof.ymmp samples/_probe/skit_01/skit_01_ir.json --tachie-motion-map samples/tachie_motion_map_library.json --skit-group-registry samples/registry_template/skit_group_registry.template.json --face-map samples/face_map_bundles/haitatsuin.json --bg-map samples/_probe/b2/palette_extract/bg_map.json --dry-run --format json` では `success=true`、`face_changes=50`、`transition_changes=10`、`motion_changes=8`、`group_motion_changes=0`、`skit_audit.summary = exact 5 / fallback 0 / manual_note 0` を確認した。warning は `bg label 'studio_blue' not found in bg_map` の 1 件のみ。最後に user report で、starter 2 件が **名前そのまま・GroupItem template・ImageItem 2 点込み**で standalone native template library に登録済みと確認した。これにより Phase 1 PASS / Phase 2 PASS / Phase 3 PASS の cautious gate が固定された
