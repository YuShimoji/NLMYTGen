...
- project: NLMYTGen
- lane: Advance
- slice: C-07 v3 拡張 (L-macro + L-research 追加) + D-02 再審査チェックリスト改訂
- next_action: Custom GPT Instructions を v3 統合プロンプトに更新 (ユーザー作業) → 実動画1本で workflow proof
- 案件モード: CLI artifact
...
## 最終検証
- last_verification_artifact: C-07 v3 プロンプト拡張 (L-macro + L-research)。統合プロンプト v3 同期。D-02 再審査チェックリスト改訂。FEATURE_REGISTRY 同期
...
- proposed: 0件
- info: 2件（C-01, C-06）
- hold: 4件（A-03, D-02 C-07v3に吸収, E-01, E-02）
...
- C-07 v3 done。マクロ演出設計 (Part 1) + 素材調達ガイド (Part 3) を追加。統合プロンプト v3 作成済み (`docs/gui-llm-setup-guide.md`)
- Custom GPT Instructions を v3 に更新する作業はユーザー側。更新後に実動画1本で workflow proof を実施
- D-02: 演出判断支援は C-07 v3 に暫定統合。独立機能にするかは proof 結果で判断。再審査チェックリスト改訂済み
- B-16/B-17 done。手動検証フィードバック待ち
- E-02: hold 継続
