...
- next_action: **視覚効果ツール選定 slice (2026-04-16 開始) が user 作業待ちで保留**。Step 1-2 完了 (assistant 成果 + user 決定記入済み)、Step 3 以降は YMM4 GUI 作業のため user actor に移譲。正本 [VISUAL-TOOL-SELECTION-2026-04-16.md](verification/VISUAL-TOOL-SELECTION-2026-04-16.md)。**user 次アクション**: (a) 既存立ち絵入り ymmp を別名複製して YMM4 で開く (例: haitatsuin_2026-04-12.ymmp → template_work_2026-04-16.ymmp)、(b) テンプレ 5 種 (skit_reaction_jump / board_list_entry / map_pan_zoom / mood_sepia_blur / intro_punch) を作成、(c) **補助 (G-22 運用)**: 主要表情を 1 フレーム透明 PNG で書き出し `samples/Mat/{speaker}_{emotion}.png` に保存、(d) `bg_map.json` / `overlay_map.json` にラベル登録 (書き出し PNG も overlay_map に)、(e) 1 本の実案件で proof → P02 追記 → hold → done 昇格。**確定した 3 方針**: ① Canva 除外 (ローカル完結維持)、② 立ち絵セットアップは踏まない (既存 `samples/Mat/新れいむ/` `samples/Mat/新まりさ/` で十分)、③ **Dual-rendering** (G-22 proposed): 立ち絵 TachieItem + 書き出し PNG overlay の両経路を用途別に使い分け。コード変更なし想定・承認判定は Step 5 proof 後 ([G22-dual-rendering-tachie-and-png-2026-04-16.md](verification/G22-dual-rendering-tachie-and-png-2026-04-16.md))。**2026-04-17 B-2 haitatsuin CLI dry-run PASS** ([B2-haitatsuin-dryrun-proof-2026-04-17.md](verification/B2-haitatsuin-dryrun-proof-2026-04-17.md)): exit 0 / fatal 0 / face_changes 18 / transition 10 / motion 3。face_map_bundles/haitatsuin.json を `migrated_tachie\` → `..\Mat\` 一括書き換え (36 パス、smile/Mouth のみ手動で `03.png` → `03b.png` 代替) + 最小 overlay_map + 10 utt IR で通過。`neutral` が prompt contract 外のため smile に置換。**立ち絵発話中非表示問題は user 先行解決済み**と 2026-04-17 に判明 ([HAITATSUIN-TACHIE-PATH-RECOVERY-2026-04-16.md](verification/HAITATSUIN-TACHIE-PATH-RECOVERY-2026-04-16.md) 5 次追記)。user の解決手順は TachieItem 修正 + YMM4 台本再読み込みで全 VoiceItem の TachieFaceParameter を再構築する方式。道 1 diff で差分ゼロを観測したのは既に修正済みファイルを比較対象にしていたため。道 2/2'/E の実施は不要。**face_map 6 表情拡張完了**: palette.ymmp から extract-template で 6 表情 (angry/sad/serious/smile/surprised/thinking) を haitatsuin.json に展開、MISSING 0。IR v2 (6 表情分散) で dry-run v2: exit 0 / face_changes **50** / warnings **4** (v1 の 10 から 6 件解消)。残 warning は全 non-fatal (霊夢 surprised palette 欠如 / neutral 保持 / idle_face / bg_map)。**2026-04-17 Step 3 補助 docs 2 件新設**: [STEP3_YMM4_TEMPLATE_CHECKLIST.md](STEP3_YMM4_TEMPLATE_CHECKLIST.md) (5 種テンプレ parameter 初期値 + チェックボックス) / [STEP3_TACHIE_RENDERING_PIPELINE.md](STEP3_TACHIE_RENDERING_PIPELINE.md) (G-22 経路 B PNG 書き出し→overlay_map 登録) + [MATERIAL_SOURCING_RULES.md](MATERIAL_SOURCING_RULES.md) §2-c-i 命名規約追記。**前セッション残件**: (A) 茶番劇テンプレ運用ガイド docs 化、(C) G-20 反転 (IsInverted) YMM4 GUI 確認。コア本開発は `repo 充足を確認済み` で新規スライスは増やさず、承認済み実装へ進む場合のみ [CORE-DEV-POST-APPROVAL-SLICES.md](verification/CORE-DEV-POST-APPROVAL-SLICES.md) §2 ゲート。
...
| 層 | 内容 | 担い手 | 完了の目安 |
| --- | --- | --- | --- |
| **主軸 (唯一)** | **演出配置自動化の実戦投入**: 本編で `overlay` / `se` (`--se-map`) / `apply-production` を使う。registry (overlay_map / se_map / bg_map) を整備し、背景アニメ 1〜2 セクションを `measure-timeline-routes` で再測定。正本 [PRODUCTION_IR_CAPABILITY_MATRIX.md](PRODUCTION_IR_CAPABILITY_MATRIX.md) / [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md) | オペレータ (GUI) + アシスタント (registry 雛形) | 1 本で演出 IR → YMM4 patch が効いたと確認 (P02 に 1 行追記) |
...
| **hold (user 判断待ち)** | **視覚効果ツール選定** (茶番劇アニメ / 図解アニメ)。主軸候補だが完了の目安が立たないため保留。トリガー: ユーザーがツール決定 | user | ツール決定後に主軸昇格 |
