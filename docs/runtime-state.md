...
- next_action: **P0 本編 v14・Block-A 通過**（2026-04-12）— [P01-phase1-operator-e2e-proof.md](verification/P01-phase1-operator-e2e-proof.md) の `p0_mainline_v14_steering_2026-04-11_a` が **PASS（YMM4 S-4）**（`samples/v14_t3_ymm4.csv`・台本読込・読込エラーなし・字幕見え方目視確認）。手順は [P0-VERTICAL-STEERING-2026-04-11.md](verification/P0-VERTICAL-STEERING-2026-04-11.md) / [P0-BLOCK-A-AND-PATH-A.md](verification/P0-BLOCK-A-AND-PATH-A.md)。**オペレータ次針**: 下記 `parallel_replan_2026_04` のトラック A/B、または [WORKFLOW.md](../WORKFLOW.md) S-5 以降。**コア本開発フェーズ T1**（[CORE-DEV-POST-APPROVAL-SLICES.md](verification/CORE-DEV-POST-APPROVAL-SLICES.md) §1 の **T1-P2-DOCSAMPLE** / **T1-RUNBOOK-GUI**）は repo 充足を確認済み。**T1 相当の新規スライスは増やさず**、承認済み実装へ進む場合のみ同ファイル §2 ゲート。**Prompt-Core-T1**: [CORE-LANE-PARALLEL-PROMPT-PACK.md](verification/CORE-LANE-PARALLEL-PROMPT-PACK.md) §3.0 早見。承認済み方針: [CORE-DEV-NEXT-IMPLEMENTATION-PLAN-DRAFT.md](verification/CORE-DEV-NEXT-IMPLEMENTATION-PLAN-DRAFT.md)（§6）。タスク設計: [CORE-DEV-TASK-DESIGN-NEXT-CYCLE.md](verification/CORE-DEV-TASK-DESIGN-NEXT-CYCLE.md)。**P2 判定**: [CORE-DEV-OPERATOR-INPUT-CHECKLIST.md](verification/CORE-DEV-OPERATOR-INPUT-CHECKLIST.md)。未承認 FEATURE は増やさない。
- parallel_replan_2026_04: **視覚最低限 + 改行／YMM4 ギャップ**の到達定義・チェックリスト・計測テンプレは [VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md](verification/VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md)。`next_action` の主文とは別軸の **並列オプション**（本編 v14 の Block-A 通過後も参照）。**P0 Block-A 通過後**またはオペレータ時間の並列で、同文書の **トラック A（演出 IR 実戦）** / **トラック B（改行ギャップ記録）** を配分する。`project-context.md` が IDE プレビューで空白になる場合は **生 Markdown で開く**（正本の針は本ファイルのまま）。エージェント依頼の **コピペ全文 Prompt・検収・親チャット返却テンプレ**は [CORE-PARALLEL-PROMPTS-AND-ACCEPTANCE-HUB-2026-04.md](verification/CORE-PARALLEL-PROMPTS-AND-ACCEPTANCE-HUB-2026-04.md)（ファイル10）に集約。
- recommended_frontier_order: 台本品質改善 → 演出配置自動化拡張 → 視覚効果実現（既定。変更可）
  - **並列の読み（2026-04-11）**: 上記フロンティア順と別に、[VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md](verification/VISUAL-MINIMUM-AND-REFLOW-PLAN-2026-04.md) の **トラック A**（最小視覚・P2 系）と **トラック B**（B-17 済み L2 と YMM4 実表示のギャップ計測）を **同時期に進めてよい**。先にオペレータ時間を取る軸は案件による（ユーザー合意で `next_action` 本文は変えず、配分のみ記録してよい）。
...
| 優先          | コード             | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | 完了の目安          |
| ----------- | --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **P0**      | —               | **Phase 1 本番 1 本（到達分解）**: 機械は **経路 A**（`validate`→`build-csv`）または **経路 B**（`diagnose-script` / GUI 診断 JSON → C-09 **任意** → `build-csv`）。NotebookLM 準拠稿では C-09 を挟まず [P01 §経路 A のみ](verification/P01-phase1-operator-e2e-proof.md) で可（正本 [P0-BLOCK-A-AND-PATH-A.md](verification/P0-BLOCK-A-AND-PATH-A.md)）。YMM4 は [P0-VERTICAL-STEERING-2026-04-11.md](verification/P0-VERTICAL-STEERING-2026-04-11.md) **Block-A**（S-4 読込エラーのみ）。接続判定を [P01-phase1-operator-e2e-proof.md](verification/P01-phase1-operator-e2e-proof.md) に 1 行以上。プラン直前の B-11 実測は [B11-workflow-proof-ai-monitoring-labor.md](verification/B11-workflow-proof-ai-monitoring-labor.md) で充足済み；別台本では [B11-pre-plan-execution-pack-2026-04-07.md](verification/B11-pre-plan-execution-pack-2026-04-07.md) に沿って新規記録を切る。S-5 は監視継続しつつ P2 背景演出試行は `next_action` に従う | **最優先・今期の主目標** |
...
| ----- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------- |
| **1** | 台本品質（候補A）   | **実装済み**: **経路 A** は `validate`→`build-csv`。**経路 B** は `diagnose-script`（B-18）→ [S1-script-refinement-prompt.md](S1-script-refinement-prompt.md)（C-09 任意）→`build-csv`（[P01](verification/P01-phase1-operator-e2e-proof.md)、[P0-BLOCK-A-AND-PATH-A.md](verification/P0-BLOCK-A-AND-PATH-A.md)）。GUI では CSV 成功時に `{stem}_script-diagnostics.json` を同梱保存可（[gui-llm-setup-guide.md](gui-llm-setup-guide.md)）。**P0**: 1 本 end-to-end で運用試行と [P01](verification/P01-phase1-operator-e2e-proof.md) 追記 | 上流が良くなると IR・配置判断も安定 |
...
- last_verification_artifact: 既定 `uv run pytest` 緑（2026-04-12、P0 Block-A 文書同期後）。フルは `NLMYTGEN_PYTEST_FULL=1 uv run pytest`（2026-04-11 時点 313 pass、subprocess CLI 統合含む）。既定の `uv run pytest` はユニット中心（`tests/conftest.py`）
- last_verification_date: 2026-04-12
