...
- slice: **G-26 motion primitive contract / screen-review preflight is the active adjustment lane**. `nod` / `exit_left` / `surprise_oneshot` now have local draft contracts under `_tmp/g26/draft_contracts/`, and `_tmp/g26/screen_review/g26_motion_primitive_compact_review.ymmp` is the current compact screen artifact. Thumbnail real-template acceptance remains a secondary lane.
- next_action: **Primary — G-26 compatibility evidence / tilt gate**. Do not keep expanding G-25 `nudge / scale / rotate` candidates. Current route readback supports option B (motion primitive first-class) and rejects single-axis cartesian variants. Treat `compatible_after` / `forbidden_after` as `unknown` until an observed two-motion chained clip exists; treat `tilt` as out-of-contract until a standalone or successful compound `.ymmp` source is registered. Use `_tmp/g26/screen_review/g26_motion_primitive_compact_review.ymmp` as the current screen-review artifact rather than producing more property variants. Do not touch `LayerSettings` / `ymmp_openability.py`, do not create Python previews, do not zero-generate `.ymmp`, and do not connect G-25/G-26 to G-24 production placement. **Secondary lanes**: real thumbnail template slot patch remains available when operator time allows; G-24 production placement remains separate and must not consume G-25 variants.
