...
- slice: **G-24 skit-group template-first -> real estate DX case IR PASS**. v1 templates are `direct_proven`; repo probe validates cleanly; production-like aliases are registered; the corrected NotebookLM script produced a real-case skit_group IR with `exact=3 / fallback=2 / manual_note=1`; `panic_shake` remains the only known distinct gap.
- next_action: **Use the validated real estate DX CSV/IR artifacts for the next production step, while keeping NotebookLM QC mandatory**. Stop broad repo-local IR exploration; pair future real/provided skit_group IRs with `samples/canonical.ymmp` and run alias-enabled `audit-skit-group`.
  - **assistant (A)**: For future NotebookLM scripts, save the user-approved/corrected text, run B-18 diagnostics or equivalent manual QC, then generate/edit skit_group IR with `motion_target: "layer:9"` and audit exact / fallback / manual_note.
  - **assistant (B)**: Keep `surprise_jump` and `deny_shake` as registry fallbacks; keep `panic_shake` as manual/new-template candidate unless user accepts a mapping.
  - **assistant (C)**: Treat `samples/haitatsuin_2026-04-12_g24_proof.ymmp` as compact template/sample proof in the current worktree; it is not currently a voice-anchored adoption corpus because the canonical group anchor is absent.
  - **user (D)**: Decide later whether `panic_shake` should become a new template, remain a manual note, or be avoided in production IR wording.
  - **shared (E)**: If exact coverage is sufficient, continue production-use hardening. If a gap appears, classify it as fallback / manual_note / new template / repo bug before creating any additional motion.
...
| 層 | 内容 | 担い手 | 完了の目安 |
| --- | --- | --- | --- |
| **主軸 (唯一)** | **G-24 茶番劇 Group template-first 運用**: 配達員などの外部素材演者を `speaker_tachie` から分離し、**canonical template → 小演出テンプレ量産 → production で template 解決 + fallback + manual note** に寄せる。正本 [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md) / [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md) | user (少数の YMM4 template 作成 / acceptance) + assistant (registry / 仕様 / fallback 整理 / production-like sample 整理) | v1 planned set 5 件は `direct_proven` 完了。次は real/probe IR で exact / fallback / manual_note が S-6（背景・演出設定）の選択負荷を減らすかを見る |
...
- last_change_relation: **2026-04-27** G-24 real estate DX skit_group IR validation. Corrected NotebookLM script is now the case input; CSV and skit_group IR artifacts exist. `audit-skit-group` returned `exact=3 / fallback=2 / manual_note=1`; `panic_shake` remains manual/new-template/IR wording avoidance candidate. NotebookLM output is now explicitly low-trust before CSV/IR generation.
...
- last_verification: **2026-04-27 push readiness**. `diagnose-script` on `samples/不動産DX_魔法の鍵とキュレーション.txt` returned 3 warnings (`NLM_STYLE_MARKER`, expected role mismatch warnings under the requested map). `build-csv` wrote `samples/不動産DX_魔法の鍵とキュレーション_ymm4.csv` with 352 rows and 18 overflow candidates. `python3 -m json.tool` checks for the registry and G-24 IR samples PASS. `audit-skit-group` returned `exact=3 / fallback=2 / manual_note=1`. `git diff --check` PASS. `uv run pytest -q -s` PASS.
