...
- slice: packaging 実装は H-02/H-03/H-04 done、H-01 は schema+proof 済みで approved のまま運用側で brief 固定化待ち。timeline (G-11〜G-13) 一巡完了。主戦場は実制作3大bottleneck (台本/演出配置/視覚効果)
- next_action: 下記「開発プラン (2026-04-06)」の Phase 1 を既定とし、着手前にユーザーが順序変更する場合のみ差し替える
...
|-------|-------------|------------------|------|
| **1** | 台本品質（候補A） | NLM 出力 → ゆっくり様式への変換・話者整合・NLM臭除去を、**Python テキスト変換**または **GUI LLM プロンプト**で 1 本の動画分 end-to-end で試す。成功基準: 手動修正時間の計測可能な削減 | 上流が良くなると IR・配置判断も安定。外部 API なしでも進められる |
...
- last_change_relation: remote sync (master = origin/master, 2026-04-06) + doc sync。直近コミット `d6b0b19`: H-03/H-04 採点実装・CLI・GUI 品質診断タブ
...
## 最終検証
- last_verification_artifact: `uv run pytest` 全件（H-03/H-04 採点テスト含む）+ master と origin の fast-forward 同期確認
...
- done: 35件（A-01, A-02, A-04, B-01〜B-09, B-11〜B-17, C-07, C-08, G-02, G-02b, G-05〜G-13, H-02, H-03, H-04）
- approved: 1件（H-01）
- proposed: 0件
- info: 2件（C-01, C-06）
...
FEATURE_REGISTRY 上 done 35 件だが、実際の動画制作ワークフロー全体に対するカバレッジは限定的。
...
done 35 件の大半は「テキスト変換パイプライン」と「spec/proof整備」に集中している。
