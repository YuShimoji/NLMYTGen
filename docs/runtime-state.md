...
- next_action: **X-2 方針で Phase 1-D 実施 + 茶番劇テンプレ先行確認**（2026-04-15 決定）。（a）Phase 1-D（palette → face_map 実パーツ一致確認）を [PROJECT_INIT_CHECKLIST.md](verification/PROJECT_INIT_CHECKLIST.md) Step 1 で通す。（b）実案件 1 本投入時に**茶番劇体テンプレ（外部素材 ImageItem + ゆっくり頭 TachieItem 重畳）を含む ymmp を選び**、既存 G-16（複数 overlay）+ G-20（group_motion）+ face/slot/motion で**どこまで到達するか先行実地確認**する。不足が顕在化したら **G-21 を `proposed` → `approved` に昇格し実装**する。Phase 1 基盤整備（registry テンプレ・初期化チェックリスト・診断マトリクス）は 2026-04-15 完了。G-20 スライス2（`mode: relative`）完了、プリセット 15 ラベル整備済み。G-20 反転（IsInverted）は YMM4 GUI 確認待ち。エフェクト選定は [samples/effect_catalog.json](../samples/effect_catalog.json)（111 効果）と [samples/EFFECT_CATALOG_USAGE.md](../samples/EFFECT_CATALOG_USAGE.md) を参照。コア本開発は `repo 充足を確認済み` で新規スライスは増やさず、承認済み実装へ進む場合のみ [CORE-DEV-POST-APPROVAL-SLICES.md](verification/CORE-DEV-POST-APPROVAL-SLICES.md) §2 ゲート。
