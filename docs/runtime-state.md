...
- **next roadmap branch prep (2026-04-27)**: After the legacy-document cleanup and template-formalism correction, the next roadmap was fixed as a gate-shaped G-24 sequence: close `delivery_nod_v1` author/export first, promote `nod` only after user-owned PASS, then widen to `deny_oneshot -> exit_left`. `docs/project-context.md` now records the formal-plan entry branches (未報告 / PASS / FAIL / 新規制作案件), and `docs/verification/PRE-PLAN-LANES-AND-CORE-DEV-2026-04-09.md` §2.3.1 mirrors the same pre-plan decision point. No FEATURE status, production route, or `next_action` changed.
- **strong legacy-plan deletion (2026-04-27)**: Old core-dev, lane prompt, parallel prompt-hub, and visual-quality packet files were deleted instead of bannered. Current state must be recovered from `AGENTS.md` -> `docs/REPO_LOCAL_RULES.md` -> this file, then from `P02` / `PRE-PLAN` only when the G-24 `delivery_nod_v1` gate needs detail.
...
- **G-24 `delivery_nod_v1` implementation pass (2026-04-27)**: Assistant implemented the non-user-owned part of the plan by re-running readiness checks. `audit-skit-group` on both `samples/canonical.ymmp` and `samples/haitatsuin_2026-04-12_g24_proof.ymmp` stayed `exact=5 / fallback=0 / manual_note=0`. `apply-production --dry-run` on the proof corpus returned `success=true`, `motion_changes=8`, and `group_motion_changes=0` when run with `--tachie-motion-map samples/tachie_motion_map_library.json`. A dry-run without that motion map fails with `MOTION_UNKNOWN_LABEL`, so the motion map is part of the required readiness command. No `direct_proven` promotion was made because user-owned YMM4 author/export is still pending.
...
- **roadmap pre-plan prep after legacy cleanup (2026-04-27)**: Updated the pre-plan intake docs so the next roadmap starts from G-24 `delivery_nod_v1` rather than the older background-animation/S6 axis. `docs/verification/PRE-PLAN-LANES-AND-CORE-DEV-2026-04-09.md` marks G-24 `nod` as the current anchor; detailed readiness vs standalone export proof lives in this file and [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md). `next_action` remains unchanged: close `delivery_nod_v1` through manual acceptance + export before widening to `deny_oneshot -> exit_left`.
- **docs cleanup / interaction failure framing correction (2026-04-26)**: Obsolete prompts, archived packets, and superseded roadmap/setup docs were deleted; `docs/INTERACTION_NOTES.md` now uses structural failure classes (`REASK_DEBT`, `BROAD_STOP`, `OPTION_COLLAPSE`, `MANUAL_PROOF_TRANSFER`, `VALUE_PATH_DRIFT`, `STATUS_DRIFT`, `DOMAIN_PACKET_COLLAPSE`). The durable rule is that interaction notes must prevent project-stalling inefficiency rather than record personal reactions. Synced `docs/ai/CORE_RULESET.md` role wording and `docs/USER_REQUEST_LEDGER.md`; G-24 `nod` next_action remains unchanged.
- **nod cautious gate readiness + packet sync (2026-04-23)**: Assistant reran `audit-skit-group` on `samples/canonical.ymmp` and `samples/haitatsuin_2026-04-12_g24_proof.ymmp`, confirmed that `delivery_nod_v1` stays `exact`, and rechecked `apply-production --dry-run` with `success=true` / `group_motion_changes=0`. Important correction: the repo still tracks only the canonical anchor plus the two starter copies (`delivery_enter_from_left_v1`, `delivery_surprise_oneshot_v1`); there is no tracked discrete `delivery_nod_v1` copy yet. The user-owned step therefore opens `samples/haitatsuin_2026-04-12_g24_proof.ymmp` or an equivalent local project in YMM4, starts from Layer 9 `GroupItem` Remark `haitatsuin_delivery_main`, and exports a YMM4 native GroupItem template named `delivery_nod_v1`. Hands-on instructions are kept in this `next_action` block and [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md), not in a standalone packet file.
- **starter batch export sync（2026-04-21）**: G-24 の初回 authoring 範囲は引き続き `delivery_enter_from_left_v1` / `delivery_surprise_oneshot_v1` の 2 件に固定。`samples/canonical.ymmp` には frame 0 の canonical anchor `haitatsuin_delivery_main` に加え、frame 306 `delivery_enter_from_left_v1` と frame 658 `delivery_surprise_oneshot_v1` の GroupItem copy が追加済みで、各 group は Layer 9 / `GroupRange=2` / 隣接 Layer 10-11 の `ImageItem` ペアを維持する。加えて user report では、この 2 件を **名前そのまま・GroupItem template・ImageItem 2 点込み**で standalone native template library へ登録済み。assistant 側は registry / preflight / P02 / handoff をこの starter export 状態に同期済み。`delivery_deny_oneshot_v1` / `delivery_exit_left_v1` / `delivery_nod_v1` は canonical corpus で exact を維持する catalog entry のまま。
...
- slice: **G-24 skit-group template-first -> `nod` cautious gate is readiness-PASS and waiting on user-owned author/export**. Starter 2 (`enter_from_left`, `surprise_oneshot`) remain `direct_proven`; this shared cycle is now focused on getting `delivery_nod_v1` through the same cautious gate without widening scope.
- next_action: **Run the G-24 `nod` cautious gate as a shared cycle**. Assistant-owned readiness and the hands-on packet are synced; the next move is the user-owned author/export step, followed by shared state sync.
  - **assistant (A)**: `audit-skit-group` is rechecked on both the canonical corpus and the proof corpus, and `delivery_nod_v1` stays `exact` with `fallback=0 / manual_note=0`. The hands-on packet is absorbed into this `next_action` block and [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md); do not depend on an untracked standalone packet file.
  - **assistant (B)**: `apply-production --dry-run` still returns `success=true` / `group_motion_changes=0`, so the `nod` path is still template-first and does not need `motion_target` / `group_motion` substitution. The key nuance is that `audit-skit-group` `exact` here means registry/preflight readiness, not export proof
  - **user (C)**: In YMM4, open `samples/haitatsuin_2026-04-12_g24_proof.ymmp` or the user's local equivalent that contains the same delivery actor setup. Start from the Layer 9 GroupItem `haitatsuin_delivery_main` (body ImageItem + face ImageItem), create a YMM4 native GroupItem template named exactly `delivery_nod_v1`, and include the two ImageItem children. Do not create a new `.ymmp`, Python artifact, or `TachieItem` setup.
  - **user (D)**: Manual acceptance means reporting the observed YMM4 result. PASS only if the template export exists, body and face move together, `nod amplitude (RepeatMove loop)` is visible but does not dominate the scene, and no `TachieItem` is included. If the only viable path needs `fallback`, `manual_note`, `motion_target`, or `group_motion`, report FAIL and replan instead of calling the export complete.
  - **shared (E)**: After user acceptance/export is confirmed, promote `skit_group.intent.nod` to `direct_proven`, keep `deny_oneshot` / `exit_left` at `template_catalog_only`, and move the next frontier to `deny_oneshot -> exit_left`
...
| 層 | 内容 | 担い手 | 完了の目安 |
| --- | --- | --- | --- |
| **主軸 (唯一)** | **G-24 茶番劇 Group template-first 運用**: 配達員などの外部素材演者を `speaker_tachie` から分離し、**canonical template → 小演出テンプレ量産 → production で template 解決 + fallback + manual note** に寄せる。正本 [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md) / [P02-production-adoption-proof.md](verification/P02-production-adoption-proof.md) | user (YMM4 template 作成 / acceptance) + assistant (registry / 仕様 / fallback 整理) | canonical 1 件 + **canonical project 内 starter 2 copies（`enter_from_left` / `surprise_oneshot`）** + manual acceptance + 1 件の production adoption proof + standalone export。その後に残り派生 4〜6 件へ広げる |
...
- last_change_relation: **2026-04-27** strong doc-excision follow-up. Stale `project-context.md` top-level roadmap blocks and old G-19/G-20 copy-prompt authority were removed rather than bannered; GUI/CLI wording and frontier authority were aligned so restart points to `runtime-state.md` first. No production artifact, FEATURE status, or `next_action` changed; the next formal plan still branches on `delivery_nod_v1` 未報告 / PASS / FAIL before widening to `deny_oneshot -> exit_left`.
...
- last_verification: **2026-04-27 strong doc-excision follow-up grep PASS**. Re-searched targeted entry/state files for stale restart pollution phrases covering old roadmap headings, GUI-only wording, outdated G-19/G-20 status wording, old cross-thread prompt authority, and broad read-order prompt labels; result: `NO_MATCHES`. `git diff --check -- AGENTS.md CLAUDE.md docs/project-context.md docs/verification/TACHIE-BODY-FACE-SWAP-PREP-2026-04-13.md` reported no whitespace errors. This block changed documentation only; no `src` or `gui` production logic changed, so pytest is not required.
