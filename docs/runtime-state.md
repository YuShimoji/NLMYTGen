...
- next_action: **assistant 先行 + user 先行の 2 系統で進める**。G-24 を user-only と読まない。
  ...
  - **user 先行 (D)**: canonical から小演出テンプレートを 1-2 件ずつ native asset として派生保存する（最初の候補は `enter_from_left` / `surprise_oneshot`）。全て `ImageItem` 重ね合わせ構成を維持 (`TachieFaceParameter` / 連番アニメ立ち絵は使わない)。assistant は保存された asset に合わせて registry / proof / atlas を都度同期する
...
- last_verification: **canonical anchor adoption の局所回帰を完走**。`python3 -m py_compile src/pipeline/capability_atlas.py scripts/build_capability_atlas.py tests/test_capability_atlas.py` PASS、`python3 -m src.cli.main audit-skit-group samples/canonical.ymmp samples/_probe/skit_01/skit_01_ir.json --skit-group-registry samples/registry_template/skit_group_registry.template.json --format json` で `status=ok / exact=5` を確認、`python3 scripts/build_capability_atlas.py` で `samples/_generated/capability_atlas.json` を再生成、`uv run python -m pytest tests/test_capability_atlas.py tests/test_skit_group_audit.py -q -s` PASS。`skit_group.canonical_anchor=direct_proven` と `skit_group.intent.*=template_catalog_only` の分離、old corpus の `SKIT_CANONICAL_GROUP_MISSING` 維持を確認
