...
- next_action: **B-2 進行中 (haitatsuin 案件)**。**復旧 B 完了 (2026-04-16)** → [HAITATSUIN-TACHIE-PATH-RECOVERY-2026-04-16.md](verification/HAITATSUIN-TACHIE-PATH-RECOVERY-2026-04-16.md)。haitatsuin ymmp の立ち絵パーツ参照を `migrated_tachie/` (placeholder clone) → `..\Mat\` (samples/Mat/ 実パーツ) に 15 パス書き換え、実ファイル全解決確認済。**user 視覚確認待ち**: YMM4 で立ち絵表示が復活しているか確認してください。次ステップ (user OK 後): B-2 Step 1-4 = (a) face_map 作成 (samples/Mat/ 相対パス版)、(b) overlay_map 作成 (reimu 6 表情 `reimu_anger/easy/panic/shobon/shocked/surprised.png`)、(c) 最初の 10 utterance 用 minimal IR 手書き (skit 方針、face + overlay 指定)、(d) validate-ir + apply-production --dry-run。**案件設定**: 案件名=haitatsuin / 視覚方針=skit / scope=10 utterance / 配達員頭=overlay_map 方式 (②-B)。**B-1 + B-3 完了 (2026-04-16)**: B-1 [B1-e2e-test-regression-proof-2026-04-16.md](verification/B1-e2e-test-regression-proof-2026-04-16.md) observational close、B-3 [B3-production-reproof-2026-04-16.md](verification/B3-production-reproof-2026-04-16.md) PASS (face 139 / 決定性 OK)。**前セッション残件**: (A) 茶番劇テンプレ運用ガイド docs 化、(C) G-20 反転 (IsInverted) YMM4 GUI 確認。エフェクト選定は [samples/effect_catalog.json](../samples/effect_catalog.json)（111 効果）と [samples/EFFECT_CATALOG_USAGE.md](../samples/EFFECT_CATALOG_USAGE.md) を参照。コア本開発は `repo 充足を確認済み` で新規スライスは増やさず、承認済み実装へ進む場合のみ [CORE-DEV-POST-APPROVAL-SLICES.md](verification/CORE-DEV-POST-APPROVAL-SLICES.md) §2 ゲート。
