...
- next_action: (1) YMM4 テンプレートファイル形式の実測 (最優先未確認項目。手順書 docs/verification/ymm4-template-measurement.md 作成済み。ユーザー操作待ち) → (2) PRODUCTION_IR_SPEC セクション2 の preset+override 候補仕様更新 → (3) Custom GPT v4 proof
- 案件モード: CLI artifact

## 主成果物
- active_artifact: NLM transcript → YMM4 CSV → ゆっくり解説動画制作ワークフロー
- artifact_surface: CLI → CSV → YMM4 台本読込 → 演出設定 → レンダリング → サムネイル → 投稿
- last_change_relation: direct (演出 IR → ymmp 変換パイプライン実装。bg+face 差し替え実機検証済み)
...
## 最終検証
- last_verification_artifact: 三層責務構造 docs 反映完了。実測手順書作成。91 PASS
...
|---|---|---|---|
| docs/verification/ymm4-template-measurement.md | 実測手順書 | 実測完了+結果反映後 | 短期 |
