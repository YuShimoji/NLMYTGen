...
- project: NLMYTGen
- lane: Advance
- slice: G-01 YMM4 プラグイン feasibility spike (最優先フロンティア)
- next_action: G-01 spike 実施 (.NET/C# で IToolPlugin 最小実装 → YMM4 にロードして動作確認)
- 案件モード: CLI artifact + YMM4 プラグイン (C#)
...
## 最終検証
- last_verification_artifact: YMM4 自動化調査完了。G-01~G-04 を FEATURE_REGISTRY に proposed 登録。ロードマップ文書 `docs/YMM4-AUTOMATION-RESEARCH.md` 作成
...
- **G-01 (最優先)**: YMM4 IToolPlugin feasibility spike。.NET/C# 環境構築 + 最小プラグインビルド + YMM4 ロード確認
- G-02: ymmp 構造解析。ユーザーから台本読込後の .ymmp サンプル提供が前提
