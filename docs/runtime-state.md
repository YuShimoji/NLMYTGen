...
- project: NLMYTGen
- lane: Advance
- slice: G-02 YMovieHelper 詳細調査 (最優先フロンティア)
- next_action: YMovieHelperLocal を起動し、CSV 入力仕様を把握。C-07 v3 出力との接続可能性を評価
- 案件モード: CLI artifact
...
## 最終検証
- last_verification_artifact: YMM4 自動化調査 (二次) 完了。YMovieHelper 発見。ロードマップ再設計: G-02 (YMovieHelper 調査) を最優先に、G-01 (IToolPlugin) を最下位に変更
...
- **G-02 (最優先)**: YMovieHelper 詳細調査。CSV 入力仕様把握 + C-07 v3 出力との接続評価
- G-05: C-07 v3 → YMovieHelper 入力変換 (build-ymh)。G-02 の結果が前提
- G-02b: 完成品 ymmp 構造解析 (研究のみ、編集しない)
- G-01: IToolPlugin spike は最下位。YMovieHelper 連携で不十分な場合の代替経路
