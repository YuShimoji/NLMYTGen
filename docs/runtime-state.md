...
- next_action: G-12 measurement packet を開き、`motion` / `transition` / `bg_anim` の write route を native template 参照か key 書き換えかで実測固定する。Packaging lane (H-01 → H-02 → H-04 → H-03) は backlog として維持
...
- last_change_relation: direct (G-11 slot patch hardening: slot contract validation + deterministic TachieItem X/Y/Zoom patch + CLI smoke)
...
- approved: 1件（H-01）
- proposed: 5件（G-12〜G-13, H-02〜H-04）
- info: 2件（C-01, C-06）
...
- **次**: G-12 timeline native-template measurement。`motion` / `transition` / `bg_anim` の write route を測って packet completion criteria を固定する
