...
- project: NLMYTGen
- lane: Advance
- slice: B-12 行バランス重視の字幕分割を実装し、CLI とテストと docs を同期する
- next_action: B-12 の post-import visual evidence を追加し、改行系 pain の減少量を再観測する
- 案件モード: CLI artifact
...
## 最終検証
- last_verification_artifact: `uv run pytest` 51 PASS、`uv run python -m src.cli.main build-csv --help` OK、`build-csv ... --max-lines 2 --chars-per-line 40 --balance-lines --stats --dry-run` 実データ確認 OK。B-12 で `--balance-lines` を追加し、2行字幕向けの自然改行 heuristics を opt-in 実装
- last_verification_date: 2026-03-31

## Evidence（CLI artifact mode）
- evidence_status: 実データ通過（2件の transcript で CSV E2E 確認済み）
- last_e2e_data: AI監視(57発話) を `--max-lines 2 --chars-per-line 40 --balance-lines` で dry-run 確認。CSV 1行内の改行保持もテストで担保
...
- done: 14件（A-01〜A-02, A-04, B-01〜B-12）
- approved: 0件
- proposed: 0件
- info: 2件（C-01, C-06）
...
- post-import の fresh visual evidence はユーザー観測ベースで取得済みだが、スクリーンショット等の保存はまだない
- B-11/B-12 により、辞書や timing ではなく字幕改行が支配的な pain だと確認し、`--balance-lines` を実装済み。残課題は YMM4 側での fresh visual evidence 更新
