...
- project: NLMYTGen
- lane: Advance
- slice: G-02 done。次フロンティア選定中
- next_action: G-05 (IR 出力プロンプト) or G-02b (ymmp 構造解析研究) の承認判断
- 案件モード: CLI artifact
...
- done: 22件（A-01〜A-02, A-04, B-01〜B-17, C-07 v3, C-08, G-02）
- approved: 0件
- proposed: 3件（G-02b ymmp構造解析研究, G-05 IR出力プロンプト, G-06 IR→YMM4接続方式決定）
- info: 2件（C-01, C-06）
...
- **G-05 (次候補)**: C-07 v4 IR 出力プロンプト。Custom GPT が構造化 IR を出力するよう改訂。G-02 完了が前提 → 前提充足
