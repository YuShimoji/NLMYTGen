...
- next_action: **X-2a 完了（2026-04-16）**。配達員 ymmp で dry-run proof 実施 → [X2A-haitatsuin-dryrun-proof-2026-04-16.md](verification/X2A-haitatsuin-dryrun-proof-2026-04-16.md)。結論: **G-21 `proposed` 維持（当面不要判定）** — 解説役 TachieItem への face 15件 / transition 3件 / motion 2件が機械適用 OK、配達員体+頭 ImageItem は YMM4 手動 GroupItem 化 + 合成 PNG 運用で足りる。Phase 1-D Step 1 も同時消化（palette から 2 キャラ分 face_map 抽出、55 パーツパス全て実在確認）。**次候補（user 判断）**: (A) 茶番劇テンプレ運用ガイドを docs/ に起こす、(B) 別の実案件で [PROJECT_INIT_CHECKLIST.md](verification/PROJECT_INIT_CHECKLIST.md) Step 2-5 を 1 本通す（主軸「演出配置自動化の実戦投入」進行）、(C) G-20 反転（IsInverted）YMM4 GUI 確認。エフェクト選定は [samples/effect_catalog.json](../samples/effect_catalog.json)（111 効果）と [samples/EFFECT_CATALOG_USAGE.md](../samples/EFFECT_CATALOG_USAGE.md) を参照。コア本開発は `repo 充足を確認済み` で新規スライスは増やさず、承認済み実装へ進む場合のみ [CORE-DEV-POST-APPROVAL-SLICES.md](verification/CORE-DEV-POST-APPROVAL-SLICES.md) §2 ゲート。
