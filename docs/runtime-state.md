...
- slice: **G-26 motion recipe pipeline v1 is implemented; visual acceptance remains gated**. `build-motion-recipes` now provides the reusable creation scheme: intent brief → effect shortlist → recipe review `.ymmp` → readback JSON → manifest. **正本ワークフロー**: [MOTION_PRODUCTION_PIPELINE.md](MOTION_PRODUCTION_PIPELINE.md) (creative slice の必読 doc、Anti-Shortcut Rules 含む)。Current active review artifact is `_tmp/g26/recipe_pipeline/g26_motion_recipe_review_v1.ymmp`: 12 proposed recipes, 12 GroupItems, 24 ImageItems, openability pass, POSIX asset paths 0, blank asset paths 0. The earlier `_tmp/g26/recipe_lab/g26_goal_motion_recipe_lab.ymmp` and `_tmp/g26/composition/演出_palette_v2.ymmp` are candidate/history artifacts, not accepted production evidence.
- next_action: **Primary — YMM4 visual review of `build-motion-recipes` v1 output**. Use `_tmp/g26/recipe_pipeline/g26_motion_recipe_review_v1_manifest.md` for frames/goals and classify each recipe per [MOTION_PRODUCTION_PIPELINE.md § 5](MOTION_PRODUCTION_PIPELINE.md#5-phase-e-visual-acceptance--library-promotion) (`pass` / `wrong motion` / `body-face drift` / `screen spacing` / `too subtle` / `too strong` / `openability` / `asset path`). Do not keep expanding G-25 `nudge / scale / rotate` candidates, and do not treat missing user-authored tilt/chain source as a reason to stop proposing motions. Treat `compatible_after` / `forbidden_after` as `unknown` until YMM4 visual acceptance promotes a chain; treat `tilt` as out-of-contract until a standalone or accepted compound `.ymmp` source is registered. The prohibition is only against Python preview/rendering, `.ymmp` zero-generation, synthetic image/effect creation, and premature G-24 production placement connection. **For creative slice agents**: read [MOTION_PRODUCTION_PIPELINE.md](MOTION_PRODUCTION_PIPELINE.md) Anti-Shortcut Rules before any motion creation work. **Secondary lanes**: real thumbnail template slot patch remains available when operator time allows; G-24 production placement remains separate and must not consume unaccepted G-26 recipes.
...
- last_change_relation: **2026-04-30** G-26 moved from one-off recipe lab to reusable motion recipe pipeline. `build-motion-recipes` now creates purpose-driven review artifacts from brief + effect catalog + template source, while keeping all recipes `proposed` until YMM4 visual acceptance.
...
- last_verification: **2026-04-30 G-26 motion recipe pipeline v1**. `uv run pytest tests/test_motion_recipe.py -q` passed (3 tests). `build-motion-recipes` generated `_tmp/g26/recipe_pipeline/g26_motion_recipe_review_v1.ymmp`; readback: YMM4 project canvas, openability pass, 36 total items, 12 recipe GroupItems, 24 ImageItems, POSIX asset paths 0, blank asset paths 0. `CameraShakeEffect` is flagged as community-plugin-dependent.
