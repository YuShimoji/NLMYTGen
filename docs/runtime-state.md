...
- slice: **G-24 skit-group template-first -> template-analyzed placement planner**. v1 templates are `direct_proven`, the repo-tracked placement source contains 5/5 templates, and `patch-ymmp --skit-group-only` can insert the real-estate DX exact/fallback cues into a CSV-imported `.ymmp` copy. The current output is raw clone transport/readback proof only: user visual check found the cues are present but spacing/composition is too rough, so production acceptance is not closed.
- next_action: **Implement template analyzer + placement planner before asking for another YMM4 proof**. Analyze `samples/templates/skit_group/delivery_v1_templates.ymmp` as source material, derive normalized actor footprint / child ImageItem offsets / relative motion / timing density, generate an analyzed placement plan, then write/readback a new patched `.ymmp`. Do not ask the user to hand-place cues or remake templates unless a missing source fact is demonstrated by readback diagnostics.
  ...
  - **assistant (C)**: Treat the existing `samples/_probe/g24/real_estate_dx_skit_group_patched.ymmp` as raw clone transport proof. The next patched output must be produced from an analyzed placement plan, not by asking the user to manually correct spacing in YMM4.
...
| 層 | 内容 | 担い手 | 完了の目安 |
| --- | --- | --- | --- |
| **主軸 (唯一)** | **G-24 茶番劇 Group template-first 運用**: 配達員などの外部素材演者を `speaker_tachie` から分離し、**repo-tracked YMM4 template source → registry 解決 → analyzed placement plan → patch-ymmp による GroupItem 自動配置**に寄せる。正本 [SKIT_GROUP_TEMPLATE_SPEC.md](SKIT_GROUP_TEMPLATE_SPEC.md) / [PRODUCTION_IR_CAPABILITY_MATRIX.md](PRODUCTION_IR_CAPABILITY_MATRIX.md) | user (少数の YMM4 template 作成 / acceptance) + assistant (registry / 仕様 / template-source 同期 / analyzer / placement planner / `.ymmp` placement 実装) | v1 planned set 5 件は `direct_proven`、repo-tracked source も 5/5 同梱済み。raw clone write/readback は通ったが production acceptance ではない。次は template analyzer + placement planner で spacing/composition を自動生成側へ戻す |
...
- last_change_relation: **2026-04-28** G-24 real-estate DX skit_group raw clone placement readback completed and image path failure fixed, but production acceptance remains open. The user-saved CSV-imported base was found at `samples/real_estate_dx_csv_import_base.ymmp`, copied to the planned probe path, aligned with row anchors, and patched to `samples/_probe/g24/real_estate_dx_skit_group_patched.ymmp`. A follow-up YMM4 check showed image association failures; root causes were stale `C:\Users\PLANNER007\...` asset paths and same-frame `deny` / `nod` template child mixing. After that fix, user visual feedback confirmed items are present but spacing/composition is too rough. The next change must add template analysis and placement planning rather than pushing template correction back to the user.
...
- last_verification: **2026-04-28 real-estate DX skit_group placement readback**. `patch-ymmp --skit-group-only` wrote `samples/_probe/g24/real_estate_dx_skit_group_patched.ymmp` from `samples/_probe/g24/real_estate_dx_csv_import_base.ymmp` + `samples/_probe/g24/real_estate_dx_skit_group_ir_aligned.json`: `Skit group placements: 5 (GroupItems inserted: 9)`. Readback PASS: Layer 9 GroupItems exist for `utt:1` frame 0, `utt:15` frame 8128, `utt:35` frame 16207, `utt:39` frame 17118, `utt:143` frame 58061; no `utt:104` insertion. Image readback PASS after fix: inserted ImageItems total 10, missing image assets 0. `json.tool` PASS for registry, source DX IR, and aligned DX IR. `uv run pytest -q -s tests/test_skit_group_placement.py tests/test_skit_group_audit.py tests/test_capability_atlas.py` PASS. `uv run pytest -q -s` PASS. `git diff --check -- src docs samples tests` PASS.
