...
- project: NLMYTGen
- lane: Advance
- slice: B-14 aggressive clause chunking を実装し、post-import visual evidence を待つ
- next_action: 字幕分割はいったん収束扱いにし、次 frontier 候補として S-6 LLM adapter の spec/ADR feasibility packet を整理する
- 案件モード: CLI artifact
...
## 最終検証
- last_verification_artifact: `uv run pytest` 56 PASS。`build-csv ... --max-lines 2 --chars-per-line 40 --balance-lines --stats --dry-run` で sample が 57 発話 → 95 行に再編され、overflow candidates は 3 件まで減少。`samples/AI監視が追い詰める生身の労働_balance_lines_ymm4.csv` を再生成済み
