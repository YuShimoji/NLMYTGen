...
| S-3 | CLI コマンド実行 | NLMYTGen が話者分離・マッピング・分割・CSV生成 |
...
- **S-6 支援 (演出 IR + LLM プロンプト)**: 演出 IR 語彙を定義し (G-02 done)、Custom GPT が構造化 IR を出力するプロンプトを提供する (G-05 done、proof 待ち)。演出 IR は背景・立ち絵・素材配置の意味ラベルを定義し、テンプレート定義で座標・ファイルパスに解決する。詳細: [PRODUCTION_IR_SPEC.md](PRODUCTION_IR_SPEC.md)
