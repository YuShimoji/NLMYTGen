...
S-5       YMM4 演出・レンダリング             手動     L3 (将来: テンプレート)
S-6       サムネイル作成                      手動     L2 (将来: テンプレート生成)
...
## S-6: サムネイル作成（手動 / 将来自動化）

1. 動画の内容に合わせたサムネイルを作成
2. 現状は手動（Canva / Photoshop 等）

### 自動化の余地
- テンプレート画像 + タイトルテキスト → Python で自動合成 (FEATURE_REGISTRY D-01)
