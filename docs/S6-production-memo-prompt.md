...
- G-24 skit_group actor（配達員など）を使う発話では `motion_target: "layer:9"` を明示し、既存 v1 intent または登録済み alias intent だけを `motion` に使う
...
**face 契約運用メモ:**
- 上記の face 一覧は Custom GPT と palette の契約である。ここを変更したら `validate-ir` / `apply-production` で `FACE_PROMPT_PALETTE_GAP` が消えるまで palette を同期する
- current IR に必要な表情が palette に無い場合は `FACE_ACTIVE_GAP` として停止する。場当たり的な prompt 再試行で流さない

**face 分布制約:**
- serious が全発話の 40% を超えないこと。超える場合は thinking / smile / surprised に分散する
- 同じ face を 4 発話以上連続させないこと。内容が変化しなくても表情に微変化を入れる
- idle_face は各セクション冒頭で明示すること (carry-forward でリセットされるため)
...
- unresolved candidate: panic_shake（強いパニックが本当に必要な場合だけ使う。後段 audit で manual_note 分類する）
...
新しい skit_group motion ラベルは作らないこと。既存 intent で足りない場合は `panic_shake` のように unresolved candidate として出し、後段の manual_note 分類に回す。
...
- index は IR 内の発話番号 (1から開始)。意味的な発話単位であり、YMM4 の行数とは一致しない場合がある
- CSV が複数行に分割されている場合は `row_start` / `row_end` で対応する CSV 行範囲を指定する (1-based, inclusive)
...
- 台本の内容を変更・要約しないでください
- IR の語彙は上記の一覧から選んでください。face は一覧にないラベル (例: neutral) を使わないこと
- bg / overlay / se は一覧にない場合、命名規則 (snake_case, 英字のみ) に従って新しいラベルを作成してよい
...
- skit_group actor 用 utterance は `motion_target: "layer:9"` 必須。使える `motion` は `enter_from_left` / `surprise_oneshot` / `nod` / `deny_oneshot` / `exit_left` / `surprise_jump` / `deny_shake` / `panic_shake` のみ。`panic_shake` は audit で manual_note 分類する前提で、安易に他テンプレへ寄せないこと
```
...
5. skit_group actor を含む IR は `python -m src.cli.main audit-skit-group samples/canonical.ymmp ir.json --skit-group-registry samples/registry_template/skit_group_registry.template.json --format text` を実行し、`exact / fallback / manual_note` を確認する。`panic_shake` 等の `manual_note` だけを新テンプレ候補 / manual note / IR wording 回避に分類する
