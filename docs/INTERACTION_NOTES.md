...
- `REVIEW_SURFACE_PACKAGING_DRIFT`: overlay/card review などの妥当性判断で、HTML preview や JSON/readback の raw wiring をそのまま user review 面として渡し、最終形だけを「これでどうですか」と提示する。作業者は内部配線や生成成功を見ているが、reviewer / manager は「何を入れたいか」「どう粒度を調整したか」「別案と比べてなぜこの案か」「どこを見て OK/NG を返すか」がないと判断精度が落ちる。スクロール過多の HTML や生 JSON は、成果物 proof であって review report ではない。
  - 予防: user review に渡す前に reviewer-facing list/map を作り、少なくとも `what to show` / `granularity choice` / `why this instead of alternatives` / `reviewer check` を場面ごとに表で示す。別案比較は当て馬にせず、採用しなかった案の実質的な弱点や統合理由を書く。HTML は visual preview、JSON/readback は machine proof、list/map は manager-facing report と明示して役割を分ける。将来 GUI がある場合はその GUI を唯一の「台所」とし、素材保管・機械 proof・レビュー判断を散らさない。
