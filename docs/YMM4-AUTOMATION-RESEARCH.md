...
演出パイプラインは三層で構成する。詳細仕様は PRODUCTION_IR_SPEC.md セクション6 を参照。

| 層 | 名称 | 入力 | 出力 | 担当 |
|----|------|------|------|------|
| 第1層 | **Writer IR** | 台本テキスト | 高水準の演出スクリプト (scene_preset + optional override) | LLM (Custom GPT) |
| 第2層 | **Template Registry** | IR の意味ラベル | 具体的なリソースパス / YMM4 native template 名 | ユーザーが辞書を整備。Python が JSON 管理 |
| 第3層 | **YMM4 Adapter** | IR + Registry + ymmp | 差し替え済み ymmp | patch-ymmp (Python)。ネイティブ機能で届かない部分のみ |

### 3.2 Writer IR (第1層)

LLM が台本から出力する演出スクリプト。

**基本方式: preset + override**
- LLM は `scene_preset=skit` のようにプリセット名を選ぶ。プリセットの defaults で bg/slot/motion/overlay 等が一括決定される
- 必要な場合のみ個別フィールド (face, bg, transition 等) で override する
- 逐次属性の全列挙を強いない

**LLM が担当するもの / しないもの:**

| Writer IR が担当 | Writer IR が担当しない |
|----------------|---------------------|
| scene_preset の選定 | ピクセル座標 |
| face / bg の override 判断 | YMM4 固有構造 |
| セクション区切り + Macro IR | ymmp の直接出力 |
| 素材調達ガイド (自然文) | ファイルパス |

### 3.3 Template Registry (第2層)

制作環境の再利用資産を辞書化する層。「どのプリセット名が何を束ねるか」を定義する。

**管理対象:**
- scene_presets: 場面テンプレートバンドル (skit/data/board 等) + ymm4_template 名参照
- characters: キャラ軸の face_map (表情 → パーツファイルパス)
- bg_map: 背景カテゴリ → 画像/動画ファイルパス
- slots: 意味座標 (left/right/center/off) → X/Y/Zoom
- se_map: 効果音ラベル → ファイルパス
...
### 3.4 YMM4 Adapter (第3層)

IR + Registry → ymmp の接着層。現在の patch-ymmp はこの初期形。

**責務分担の原則:**
- YMM4 native template で解決できるもの (エフェクト、アニメーション、場面バンドル) はネイティブに委ねる
- Adapter は face/bg/slot 等の JSON キー置換レベルの差し替えに集中する
- YMM4 の既存機能を Python で再発明しない
- ネイティブ機能の範囲は実測で確認する (セクション7 参照)
...
[台本テキスト]
    | NLMYTGen build-csv
    v
[YMM4 CSV] ---------> [YMM4 台本読込] -> 音声生成 + 字幕配置 -> [ymmp]
    |                                                              |
    | Writer IR (Custom GPT v4)                                    |
    v                                                              |
[演出 IR JSON]                                                     |
    |                                                              |
    | Template Registry (JSON) で意味ラベル → リソース解決          |
    v                                                              |
[解決済み IR]                                                       |
    |                                                              |
    | YMM4 Adapter (patch-ymmp) で face/bg/slot 差し替え           |
    v                                                              v
[差し替え済み ymmp] -----> [YMM4 で開く: 確認 + 微調整]
```
...
| 工程 | 担当層 | Actor |
|------|--------|-------|
| 演出 IR の語彙定義 | PRODUCTION_IR_SPEC (docs) | assistant |
| 台本 → Writer IR の生成 | 第1層 | LLM (Custom GPT) + ユーザー |
| Template Registry の辞書管理 | 第2層 | ユーザー (ラベル付け) + assistant (JSON 構造) |
| IR → ymmp の接着 | 第3層 (Adapter) | tool (patch-ymmp) |
| YMM4 native template の登録 | YMM4 内部 | ユーザー |
| **素材ファイルの作成・収集** | 人間 | **手動** |
| **テンプレートの新規設計** | 人間 | **手動** |
| **仮組立後の微調整** | 人間 | **手動** |
...
- **テンプレートは独立ファイルではなく、`ItemSettings.json` の `Templates` 配列に JSON オブジェクトとして保存される** (2026-04-03 実測)
- **テンプレート 1 件の構造**: `{Name, Path (フォルダ階層配列), Group (アイテム種別), KeyGesture, FPS/Width/Height/Hz, SceneId/SceneName, VerticalLine, LayerSettings, Items (アイテム配列), Layer}`
- **テンプレートの Items 配列内のアイテムは ymmp の Items と同一 JSON 構造** (2026-04-03 実測)
- **ImageItem テンプレートに VideoEffects が完全保持される** (StripeGlitchNoise, RectangleGlitchNoise, RandomRotateEffect を確認。2026-04-03 実測)
- **VoiceItem テンプレートに VoiceCache (音声キャッシュ) が保持される** (25,632文字のBase64。テンプレート貼り付け時に音声を維持する仕組み。2026-04-03 実測)
- **テンプレートの保存先は YMM4 バージョンごとのフォルダ** (`user/setting/4.51.0.1/` 等。2026-04-03 実測)

### 推測だが有力

- テンプレートに複数アイテムをバンドルできる (公式 FAQ に「テンプレートからアイテムを追加」の記述あり。今回の実測では Items: list[1] の単一アイテムテンプレートのみ確認)
- テンプレートダブルクリックでタイムラインに追加 (v4.29.0.0 リリースノート)
- エクスポート/インポートは ItemSettings.json 全体のやりとりと推測 (実測では独立エクスポートファイルは未生成)

### 未確認なので実測が必要

| 項目 | 影響範囲 | 優先度 |
|------|---------|--------|
| 複数アイテムをバンドルしたテンプレート (Items: list[2+]) の実例 | scene_preset バンドルの実現可能性 | 高 |
| 表情アイテムの ymmp JSON 構造 | face 差し替えの代替経路 (TachieFaceParameter 直接 vs 表情アイテム挿入) | 高 |
| VideoEffects の $type ごとの内部パラメータ構造 | motion/transition/bg_anim を Adapter で解決できるかの判断 | 中 |
| テンプレートのインポート/エクスポート機能の具体的な出力形式 | Python でのテンプレート登録・配布可能性 | 中 |
