...
YMM4-native seed or explicitly promoted carrier, an HTML/SVG visual prototype approved before
...
| 2026-03-30 | S-6 トピック分析は stdlib 制約内では精度不足 | パターンマッチ / 軽量NLP / やらない | パターンマッチ30-50%、NLP 40-60%+当時の project scope 違反。LLM アダプター方式に転換予定 |
...
| 2026-04-02 | 正本ドキュメント5件を演出IR主軸に更新 | 修正 | README/旧ツール別入口/WORKFLOW/AUTOMATION_BOUNDARY/INVARIANTS から「CSV変換専用ツール」旧理解を除去。再開時の旧理解引き戻しを構造的に防止 |
...
| ID | 旧アイデア | 移行先 |
|----|----------|--------|
| IP-02 | Web UI 化 | INVARIANTS / FEATURE_REGISTRY の現行スコープ外 |
...
- Next frontier: return to the `G27_PublicVsBrokerDB` carrier / slot-fill path. Do not design an anchored slot contract or implement slot-fill before a stable YMM4-native carrier artifact exists or an existing diagnostic carrier is explicitly promoted. After carrier receipt or promotion, assistant should read back required `G27PBD_*` item names, short Remarks, fixed card counts, `G27PBD_Arrow` existence, and geometry-free patch feasibility.
...
- Restart path: read `AGENTS.md` -> `docs/REPO_LOCAL_RULES.md` -> `docs/runtime-state.md`. The live `next_action` is waiting for a stable `G27_PublicVsBrokerDB` YMM4-native carrier artifact or explicit diagnostic-carrier promotion, not a scene-generation or render slice.
