...
| episode-002-verified-local-evidence-render-v1 | REAL_INPUT_INTERNAL_PILOT / HEADLESS_RENDER_FINALIZATION | accepted/internal-render-validated | The ignored local project and original MP4 are hash-bound and machine-validated, full video/audio decode passed, an ignored review proxy and tracked internal review package exist, and the batch collector is hardened without changing the successful operator result. Next is an advisory milestone integration audit. |
| integrity-triage | INTEGRITY_TRIAGE | paused | Full-suite drift remains nonblocking for targeted slices. |
| control-boundary-correction | INTEGRITY_TRIAGE | completed/return-to-product | Repo-side supervisor control plane remains removed; product work has advanced through verified-local-evidence internal render validation to the advisory milestone integration audit. |
| cross-device-restart-handoff | INTEGRITY_TRIAGE | completed/remote-seal | Current restart context points to `codex/episode-002-verified-local-evidence-render-v1`, state `episode-002-verified-local-evidence-internal-render-validated-v1`, and the advisory milestone integration audit. |
