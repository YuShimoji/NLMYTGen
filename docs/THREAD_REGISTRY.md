...
| episode-002-verified-local-evidence-render-v1 | REAL_INPUT_INTERNAL_PILOT / HEADLESS_OPERATOR_BATCH_PREP | accepted/operator-batch-ready | A tracked six-source bundle, 9/9 claim ledger, Japanese 9-cue script, strict canonical/derived CSV pair, VoiceItem-preserving generator, static non-final project contract, and five-action operator batch are preflight-passed; actual YMM4 import/render is pending. |
| integrity-triage | INTEGRITY_TRIAGE | paused | Full-suite drift remains nonblocking for targeted slices. |
| control-boundary-correction | INTEGRITY_TRIAGE | completed/return-to-product | Repo-side supervisor control plane remains removed; product work has advanced to the verified-local-evidence manual render gate. |
| cross-device-restart-handoff | INTEGRITY_TRIAGE | completed/remote-seal | Current restart context points to `codex/episode-002-verified-local-evidence-render-v1`, state `episode-002-verified-local-evidence-operator-batch-ready-v1`, and the one-shot operator batch. |
