...
| new-banknote-authoritative-source-script-v1 | INPUT_API_HUB / CANONICAL_SCRIPT_EDITORIAL_CONVERGENCE | completed/audited-as-selective-candidate | Candidate tip `833717f` remains unchanged. Its editorial deep-audit, historical YMM4 observation, and unselected A/B/C proposal paths are selective inputs; its current state, approved-README variant, and predecessor Operator Batch are not current authority. |
...
| new-banknote-existing-yymm4-evidence-revalidation-v1 | CONTENT_PROVENANCE / YMM4_EVIDENCE_RECONCILIATION | completed/current-lineage-compatible-primary | Primary tip `5e50ff7` remains unchanged and authoritative for explicit approval, T00–T07, current-lineage YMM4 revalidation, and the five-action Operator Batch. |
| new-banknote-successor-branch-integration-audit-v1 | INTEGRATION_COORDINATOR / SUCCESSOR_BRANCH_AUDIT | completed/selective-integration-ready | All 20 unique commits and 84 side-path entries / 77 union paths are classified with zero unknowns. Recommendation is path-selective construction: 27 integrate, 2 historical, 8 regenerate, 14 exclude; source branches remain unmodified and Route A remains unselected. |
