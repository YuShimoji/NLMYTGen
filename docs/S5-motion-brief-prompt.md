...
- LLM は effect chain や parameter を **書かない**。それは CLI が brief から解決する
...
入力: 「驚きすぎて画面に色ずれが走り、同時に小さく跳ねる」

出力:
```json
{
  "goal_id": "shocked_chromatic_pop",
  "motion_goal": "Shock reaction with chromatic aberration spike and small pop.",
  "emotion": "surprise",
  "intensity": "strong",
  "duration_frames": 36,
  "reset_policy": "returns_to_neutral",
  "forbidden_patterns": ["wrong motion", "too busy", "body-face drift"]
}
```
```
