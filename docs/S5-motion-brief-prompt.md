...
## Novel goal_id を要求する場合 (brief 内完全定義)

**Slice 4 (2026-05-01) 以降**: novel goal_id は brief entry 内で完全定義可能。`build-motion-recipes` の `_resolve_brief_recipes` が拡張され、`DEFAULT_RECIPE_PRESETS` に登録されていない goal_id でも、brief entry が必須キー (`motion_goal` / `emotion` / `intensity` / `duration_frames` / `reset_policy` / `forbidden_patterns` / `effect_candidates`) を含めば accept される。preset 拡張のためのコード変更は不要。

novel goal の brief entry には、必須キーに加えて以下の **route / effect フィールド** を追加する:

```jsonc
{
  "goal_id": "<snake_case_unique>",
  "motion_goal": "<英 1 文>",
  ...
  // route values (必要なものだけ。指定しないと static = 1 値のみ扱い)
  "rotation_values": [0.0, ...],          // Rotation route 度。AnimationType="直線移動" で keyframes 等間隔線形補間
  "y_delta_values": [0.0, ...],           // Y route の増分。base Y (462.5) からの delta
  "zoom_delta_values": [0.0, ...],        // Zoom route の増分。base Zoom (103.8) からの delta

  // effect 構成 (必須: 少なくとも effect_candidates)
  "effect_names": ["CenterPointEffect", "..."],   // 実際に attach する effect。CenterPointEffect は自動先頭付与
  "effect_candidates": ["..."]                    // 必須。空だと build で MOTION_RECIPE_EFFECT_SHORTLIST_EMPTY エラー
}
```
...
- effect_names / effect_candidates で指定する effect は [`effect_catalog.json`](../samples/effect_catalog.json) に存在するものだけ
- 数値範囲は [`MOTION_RECIPE_LOOP_TIMING.md`](MOTION_RECIPE_LOOP_TIMING.md) の intensity scaling formula に従う
- emotion カテゴリは 8 種から逸脱しない (novel goal でも)

### 既存 preset への昇格

brief 内 novel goal が visual acceptance pass し、複数案件で再利用される見込みなら、`DEFAULT_RECIPE_PRESETS` に preset 化することで次回以降 brief を簡潔化できる (goal_id だけで参照可能になる)。これは Phase E (library promotion) の選択工程であって必須ではない。novel goal を brief 内で持ち回しても CLI は問題なく動く。
...
### 例 3: novel な複合要素 (brief 内完全定義、Slice 4 以降の推奨形式)

入力: 「わー、と二段階で跳ねてはしゃぐ」

出力 (brief 1 entry のみ。preset 拡張不要):
```json
{
  "goal_id": "excited_double_jump",
  "motion_goal": "Two consecutive jumps with mid-rest for excited / hyped reaction.",
  "emotion": "happiness",
  "intensity": "strong",
  "duration_frames": 90,
  "reset_policy": "returns_to_neutral",
  "forbidden_patterns": ["wrong motion", "too subtle"],
  "y_delta_values": [0.0, -45.0, 5.0, -35.0, 0.0],
  "effect_names": ["CenterPointEffect"],
  "effect_candidates": ["CenterPointEffect", "JumpEffect", "RepeatMoveEffect"]
}
```

このエントリを `samples/recipe_briefs/<id>.json` の `recipes` 配列に追記すれば、`build-motion-recipes` がそのまま consume する。`motion_recipe.py` の編集は不要。
```
