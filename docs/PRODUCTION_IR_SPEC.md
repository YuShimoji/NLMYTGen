...
| `motion_target` | string \| object | 任意 | motion の適用先。省略または `"speaker"` で speaker の TachieItem (既定)。`"layer:N"` または `{"layer": N}` で指定レイヤーの ImageItem/GroupItem に適用 |
...
- 対象レイヤーに単一アイテムがある場合、TachieItem と同様に発話タイミングで分割して区間ごとに VideoEffects を書き込む
- 対象レイヤーに複数アイテムがある場合、発話と時間的に重なるアイテムに VideoEffects を適用する
