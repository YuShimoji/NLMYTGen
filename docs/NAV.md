...
- [CURRENT_BASIS_REVIEW_CONTRACT.md](CURRENT_BASIS_REVIEW_CONTRACT.md) — **現行production入口gate**: runtime-stateのmachine projection、Evidence/Ruleで閉じる判断、人に残す一件、退役した旧契約をGUI先頭で分類し、未束縛のsuccessor episodeからmanifest/dry-run/renderへ進む経路をfail-closedにする
