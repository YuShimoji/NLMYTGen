...
- [DEVELOPMENT_AUDIO_SAFETY.md](DEVELOPMENT_AUDIO_SAFETY.md) — **開発時の静音契約**: guarded browser、DOM mute、project-owned Core Audio session、再生deny、fail-safeの正本
...
- [CURRENT_BASIS_REVIEW_CONTRACT.md](CURRENT_BASIS_REVIEW_CONTRACT.md) — **現行production入口gate**: runtime-stateのmachine projectionをexact Source / Artifactとして非上書きDecisionCaseへ束縛し、Evidence / Rule、未回答のHuman correction一件、未作成のResulting artifactをGUI先頭で分類する。未束縛のsuccessor episodeからmanifest/dry-run/renderへ進む経路はfail-closed
- [CONTENT_TRANSFORMATION_PROVENANCE.md](CONTENT_TRANSFORMATION_PROVENANCE.md) — **content lineage受入契約**: raw/input → claim/evidence → editorial → approval → serialization のstage identity、M/E/S/U change class、cue-level meaning-unit lineage、no-silent-change lock、successor approval receiptを定義する
