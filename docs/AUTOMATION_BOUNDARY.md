...
3. **それは「YMM4 の設定を決める」作業か？** → YMM4 内部 (L3)
   ...
   - Python から .ymmp をゼロから生成する経路は存在しない。S-4（CSV 台本読込・音声合成・字幕配置・発音情報生成）を Python で置き換えない。ただし台本読込後の ymmp に対する限定的な後段適用 (IR に基づく face/bg/slot/se/overlay/skit_group placement) は patch-ymmp 系で実施する
