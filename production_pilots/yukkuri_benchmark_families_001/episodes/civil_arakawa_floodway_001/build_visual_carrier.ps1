...
if (-not $InputYmmp) {
    $InputYmmp = Join-Path $localOutputDir 'civil_arakawa_floodway_001_baseline.ymmp'
}
if (-not $OutputYmmp) {
    $OutputYmmp = Join-Path $localOutputDir 'civil_arakawa_floodway_001_visual_v001.ymmp'
}
...
$rendererProfile = Join-Path ([System.IO.Path]::GetTempPath()) (
    'nlmytgen-civil-carrier-edge-' + [System.Diagnostics.Process]::GetCurrentProcess().Id
)
New-Item -ItemType Directory -Path $rendererProfile -Force | Out-Null

$labels = [ordered]@{
    route_question = '01_route_question'
    flood_trigger = '02_flood_trigger'
    construction_timeline = '03_construction_timeline'
    flow_split = '04_flow_split'
    observed_effect = '05_observed_effect'
    present_risk_recap = '06_present_risk_recap'
}
