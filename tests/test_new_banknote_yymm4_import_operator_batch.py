...
def test_tracked_operator_batch_is_byte_deterministic_and_preflight_passes(
    ...
    assert permissions["operator_batch_preparation"] is True
    assert permissions["later_user_manual_import_observation"] is True
    for key in ("wording_or_claim_revision", "worker_yymm4_operation", "render", "production", "publication", "rights_action"):
        assert permissions[key] is False

    approved_hashes = receipt["approved_file_hashes"]
...
def test_operator_surfaces_are_utf8_safe_launch_bounded_and_private() -> None:
    ...
    assert run_text.index("if ($PreflightOnly)") < run_text.index("$Ymm4 = Resolve-Ymm4Exe")
    ...
    assert manifest["manual_action_count"] == 5
    ...
    tracked_paths = [
        DEFAULT_PILOT_DIR / APPROVAL_RECEIPT_FILENAME,
...
def test_collect_only_powershell_fixture_validates_9_3_6_and_timing(
    ...
    assert completed.stdout.splitlines() == [
        ...
        "3. pronunciation_notes: cue_006: ok",
    ]
    ...
    assert result["operator_observation"]["pronunciation_or_clipping_notes"] == "cue_006: ok"
