...
def test_content_lock_covers_current_script_yymm4_and_visual_identities() -> None:
    ...
    groups = lock["locked_artifact_groups"]
    secondary_locked = {
        row["repo_relative_path"].split(
            "new_banknote_security_notebooklm_001/", 1
        )[1]: row["sha256"]
        for row in groups["secondary_D00_D10_deep_audit_inputs"]
        + groups["historical_yymm4_observation"]
        + groups["visual_proposal_definitions"]
    }
    assert secondary_locked == LOCKED_ARTIFACT_HASHES
    for relative, digest in secondary_locked.items():
        assert hashlib.sha256(
            (DEFAULT_PILOT_DIR / relative).read_bytes()
        ).hexdigest() == digest
    assert lock["primary_approval"] == {
        "receipt_id": "new-banknote-script-option-a-approval-v1",
        "receipt_sha256": PRIMARY_AUTHORITY_HASHES[
            "human_script_approval_receipt.json"
        ],
        "status": "valid",
        "approved_commit": "b05eb3867caabda496fb9a0070d230a4e81aea01",
        "approved_file_hashes": PRIMARY_APPROVED_FILE_HASHES,
        "approved_hash_count": 8,
        "role": "sole_current_content_authority",
    }
    assert lock["authority_resolution"]["content_lineage"]["current"] == (
        "primary_T00_T07"
    )
    assert lock["authority_resolution"]["editorial_provenance"] == {
    ...
    assert lock["authority_resolution"]["yymm4"]["current"] == (
        "primary_existing_evidence_revalidation"
    )
    assert lock["authority_resolution"]["yymm4"]["historical_predecessor"] == (
        "candidate_tracked_import_observation"
    )
    assert lock["authority_resolution"]["operator_batch"]["current"].startswith(
        "primary_five_action"
    )
    assert lock["authority_resolution"]["visual"] == {
        "package": "candidate_A_B_C_proposal_only",
        "recommendation_status": "recommended_not_selected",
        "selected_route": None,
    ...
    local_yymm4 = lock["historical_observation_local_evidence_availability"]
    assert local_yymm4["status"] == (
        "present_and_hash_matched_same_machine_non_authoritative"
    )
    assert local_yymm4["authority_role"] == (
        "historical_predecessor_availability_only"
    )
...
def test_validation_readback_matches_target_state_and_acceptance() -> None:
    ...
    assert readback["authority_result"]["content_lineage"]["current"] == (
        "primary_T00_T07"
    )
    assert readback["authority_result"]["visual"]["selected_route"] is None
