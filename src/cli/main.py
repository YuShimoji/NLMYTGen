...
def main(argv: list[str] | None = None) -> int:
    ...
    p_surface_alignment = subparsers.add_parser(
        "build-surface-alignment-pack",
        help="Build a local/offline cross-surface alignment pack for GUI, import preview, and thumbnail proof",
    )
    p_surface_alignment.add_argument("--package", required=True, help="Episode package directory")
    p_surface_alignment.add_argument(
        "--output",
        help="Output directory (default: <package>/surface_alignment_pack)",
    )
    p_surface_alignment.add_argument(
        "--artifact-id",
        default="episode_002_surface_alignment_across_gui_import_thumbnail_v1",
        help="Surface alignment artifact id",
    )
    p_surface_alignment.add_argument(
        "--format",
        choices=["text", "json"],
    ...
    p_review_cockpit_compact = subparsers.add_parser(
        "build-review-cockpit-compact",
        help="Build a compact dark review cockpit from the focused brief and reviewer packet",
    )
    p_review_cockpit_compact.add_argument("--package", required=True, help="Episode package directory")
    p_review_cockpit_compact.add_argument(
        "--focused-brief-dir",
        help="Input focused review brief directory (default: <package>/focused_review_brief)",
    )
    p_review_cockpit_compact.add_argument(
        "--reviewer-packet-dir",
        help="Input surface alignment review packet directory (default: <package>/surface_alignment_review_packet)",
    ...
    p_review_cockpit_compact.add_argument(
        "--output",
        help="Output directory (default: <package>/review_cockpit_compact)",
    )
    p_review_cockpit_compact.add_argument(
        "--artifact-id",
        default="episode_002_review_cockpit_compact_v1",
        help="Compact review cockpit artifact id",
    )
    p_review_cockpit_compact.add_argument(
        "--format",
        choices=["text", "json"],
    ...
    p_review_layout_research = subparsers.add_parser(
        "build-review-layout-research",
        help="Build the episode 002 layout research and wireframe benchmark packet",
    )
    p_review_layout_research.add_argument("--package", required=True, help="Episode package directory")
    p_review_layout_research.add_argument(
        "--cockpit-dir",
    ...
    p_review_layout_research.add_argument(
        "--output",
        help="Output directory (default: <package>/review_layout_research)",
    )
    p_review_layout_research.add_argument(
        "--artifact-id",
        default="episode_002_review_layout_research_and_pattern_benchmark_v1",
        help="Layout research artifact id",
    )
    p_review_layout_research.add_argument(
        "--format",
        choices=["text", "json"],
    ...
    p_review_console_redesign = subparsers.add_parser(
        "build-review-console-redesign-prototype",
        help="Build the episode 002 review judgment console redesign prototype",
    )
    p_review_console_redesign.add_argument("--package", required=True, help="Episode package directory")
    p_review_console_redesign.add_argument(
        "--current-split-view-dir",
        help="Input current split-view prototype directory (default: <package>/split_view_decision_evidence_prototype)",
    )
    p_review_console_redesign.add_argument(
        "--second-pass-dir",
        help="Input second-pass layout benchmark directory (default: <package>/review_layout_second_pass)",
    ...
    p_review_console_redesign.add_argument(
        "--output",
        help="Output directory (default: <package>/review_console_redesign_prototype)",
    )
    p_review_console_redesign.add_argument(
        "--artifact-id",
        default="episode_002_review_console_redesign_prototype_v1",
        help="Review console redesign prototype artifact id",
    )
    p_review_console_redesign.add_argument(
        "--explicit-yymm4-observation",
        action="store_true",
    ...
    p_japanese_graphic_review_console = subparsers.add_parser(
        "build-japanese-graphic-review-console",
        help="Build the episode 002 Japanese graphical review console",
    )
    p_japanese_graphic_review_console.add_argument("--package", required=True, help="Episode package directory")
    p_japanese_graphic_review_console.add_argument(
        "--current-console-dir",
        help="Input current primary artifact console directory (default: <package>/primary_artifact_review_console)",
    )
    p_japanese_graphic_review_console.add_argument(
        "--second-pass-dir",
        help="Input second-pass layout benchmark directory (default: <package>/review_layout_second_pass)",
    ...
    p_japanese_graphic_review_console.add_argument(
        "--lane-map-path",
        help="Project lane map path (default: docs/PROJECT_LANES.md)",
    )
    p_japanese_graphic_review_console.add_argument(
        "--output",
        help="Output directory (default: <package>/japanese_graphic_review_console)",
    )
    p_japanese_graphic_review_console.add_argument(
        "--artifact-id",
        default="episode_002_japanese_graphic_review_console_v1",
        help="Japanese graphical review console artifact id",
    )
    p_japanese_graphic_review_console.add_argument(
        "--explicit-yymm4-observation",
        action="store_true",
    ...
    p_editing_operations_readiness_pack.add_argument("--package", required=True, help="Episode package directory")
    p_editing_operations_readiness_pack.add_argument(
        "--output",
        help="Output directory (default: <package>/editing_operations_readiness_pack)",
    )
    p_editing_operations_readiness_pack.add_argument(
        "--artifact-id",
        default="episode_002_editing_operations_readiness_pack_v1",
        help="Editing operations readiness artifact id",
    )
    p_editing_operations_readiness_pack.add_argument(
        "--format",
        choices=["text", "json"],
    ...
    try:
        ...
        elif args.command == "build-review-cockpit-compact":
...
def _cmd_build_surface_alignment_pack(args: argparse.Namespace) -> int:
    """Build a local/offline surface alignment pack."""
    from src.pipeline.surface_alignment_pack import build_surface_alignment_pack
...
def _cmd_build_review_cockpit_compact(args: argparse.Namespace) -> int:
    """Build a compact dark review cockpit from focused and source review records."""
    from src.pipeline.review_cockpit_compact import build_review_cockpit_compact

    readback = build_review_cockpit_compact(
        package_dir=getattr(args, "package"),
        focused_brief_dir=getattr(args, "focused_brief_dir", None),
        reviewer_packet_dir=getattr(args, "reviewer_packet_dir", None),
        output_dir=getattr(args, "output", None),
        artifact_id=getattr(args, "artifact_id"),
    )
    ...
    if fmt == "json":
        ...
        print(f"decision_options: {readback.get('decision_options')}")
        print(f"surface_status_count: {readback.get('surface_status_count')}")
        print(f"primary_section_count: {readback.get('primary_section_count')}")
        print(f"visible_card_count: {readback.get('visible_card_count')}")
        print(f"layout_bloat_status: {readback.get('layout_bloat_status')}")
        print(f"source_record_policy: {readback.get('source_record_policy')}")
        print(f"external_dependency_status: {readback.get('external_dependency_status')}")
        print(f"white_background_status: {readback.get('white_background_status')}")
        print(f"temporary_review_copy_status: {readback.get('temporary_review_copy_status')}")
        print(f"primary_machine_readable: {readback.get('primary_machine_readable')}")
        print(f"primary_human_review: {readback.get('primary_human_review')}")
        print(f"markdown_fallback: {readback.get('markdown_fallback')}")
...
def _cmd_build_review_layout_research(args: argparse.Namespace) -> int:
    """Build the episode 002 layout research and wireframe benchmark packet."""
    from src.pipeline.review_layout_research import build_review_layout_research
...
def _cmd_build_review_console_redesign_prototype(args: argparse.Namespace) -> int:
    """Build the episode 002 review judgment console redesign prototype."""
    ...
    readback = build_review_console_redesign_prototype(
        package_dir=getattr(args, "package"),
        current_split_view_dir=getattr(args, "current_split_view_dir", None),
        second_pass_dir=getattr(args, "second_pass_dir", None),
        guided_flow_dir=getattr(args, "guided_flow_dir", None),
        cockpit_dir=getattr(args, "cockpit_dir", None),
        reviewer_packet_dir=getattr(args, "reviewer_packet_dir", None),
    ...
    if fmt == "json":
        ...
        print(f"primary_recommendation: {readback.get('primary_recommendation')}")
        print(f"initial_visible_text_reduction: {readback.get('initial_visible_text_reduction')}")
...
def _cmd_build_japanese_graphic_review_console(args: argparse.Namespace) -> int:
    """Build the episode 002 Japanese graphical review console."""
    ...
    readback = build_japanese_graphic_review_console(
        ...
        cockpit_dir=getattr(args, "cockpit_dir", None),
        reviewer_packet_dir=getattr(args, "reviewer_packet_dir", None),
        lane_map_path=getattr(args, "lane_map_path", None),
        output_dir=getattr(args, "output", None),
        artifact_id=getattr(args, "artifact_id"),
        explicit_yymm4_observation=bool(getattr(args, "explicit_yymm4_observation", False)),
    )
    ...
    if fmt == "json":
        ...
        print(f"critical_issue: {readback.get('critical_issue')}")
        print(f"primary_recommendation: {readback.get('primary_recommendation')}")
        print(f"operational_controls: {readback.get('operational_controls')}")
        print(f"main_surface_type: {readback.get('main_surface_type')}")
        print(f"graphical_elements: {readback.get('graphical_elements')}")
        print(f"primary_ui_language: {readback.get('primary_ui_language')}")
        print(f"english_primary_headings: {readback.get('english_primary_headings')}")
        print(f"free_text_role: {readback.get('free_text_role')}")
        print(f"same_shape_card_grid_primary: {readback.get('same_shape_card_grid_primary')}")
        print(f"explanatory_cards_in_main_surface: {readback.get('explanatory_cards_in_main_surface')}")
        print(f"evidence_front_stage_card_row: {readback.get('evidence_front_stage_card_row')}")
        print(f"evidence_visible_outside_drawer: {readback.get('evidence_visible_outside_drawer')}")
        print(f"detail_drawer_role: {readback.get('detail_drawer_role')}")
        print(f"gate_text_bounded: {readback.get('gate_text_bounded')}")
        print(f"project_lanes_recorded: {readback.get('project_lanes_recorded')}")
        print(f"stop_rule_recorded: {readback.get('stop_rule_recorded')}")
        print(f"source_records_secondary: {readback.get('source_records_secondary')}")
        print(f"internal_artifact_ids_in_primary_copy: {readback.get('internal_artifact_ids_in_primary_copy')}")
        print(f"primary_machine_readable: {readback.get('primary_machine_readable')}")
        print(f"primary_human_review: {readback.get('primary_human_review')}")
...
def _cmd_build_editing_operations_readiness_pack(args: argparse.Namespace) -> int:
    """Build the episode 002 editing operations readiness package."""
    ...
    if fmt == "json":
        ...
        print(f"status: {readback.get('status')}")
        print(f"primary_review_file: {readback.get('primary_review_file')}")
        print(f"operation_count: {readback.get('operation_count')}")
        print(f"scene_count: {readback.get('scene_count')}")
        print(f"timing_model_status: {readback.get('timing_model_status')}")
        print(f"voice_subtitle_operation_status: {readback.get('voice_subtitle_operation_status')}")
        print(f"visual_slot_map_status: {readback.get('visual_slot_map_status')}")
        print(f"yymm4_protocol_status: {readback.get('yymm4_protocol_status')}")
        print(f"yymm4_readback_schema_status: {readback.get('yymm4_readback_schema_status')}")
        print(f"invented_real_content: {readback.get('invented_real_content')}")
