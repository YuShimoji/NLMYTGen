...
from src.pipeline.content_transformation_lineage import (
    APPROVAL_RECEIPT_ID,
    APPROVED_HASHES as LINEAGE_APPROVED_HASHES,
...
APPROVAL_RECEIPT_FILENAME = "human_script_approval_receipt.json"
LINEAGE_LOCK_FILES = (
...
OPERATOR_DIRNAME = "yymm4_operator_batch"
README_FILENAME = "README_OPERATOR_BATCH.md"
MANIFEST_FILENAME = "operator_batch_manifest.json"
EXPECTED_CONTRACT_FILENAME = "expected_result_contract.json"
PREFLIGHT_FILENAME = "preflight_readback.json"
RETURN_TEMPLATE_FILENAME = "operator_return_template.md"
RUN_SCRIPT_FILENAME = "run_new_banknote_yymm4_batch.ps1"
COLLECT_SCRIPT_FILENAME = "collect_new_banknote_yymm4_result.ps1"
...
LOCAL_OBSERVATION_FILENAME = "operator_observation.local.json"
...
APPROVED_FILES = tuple(LINEAGE_APPROVED_HASHES)
EXPECTED_APPROVED_HASHES = dict(LINEAGE_APPROVED_HASHES)
...
def _load_contract_inputs(pilot: Path) -> dict[str, Any]:
    ...
    lineage_validation = validate_content_lineage_package(pilot)
    if lineage_validation.get("status") != "passed":
        raise ValueError(
            "CONTENT_LINEAGE_OR_APPROVAL_INVALID:"
            + ",".join(
                str(item) for item in lineage_validation.get("failed_checks", [])
            )
        )
    approval_receipt = _read_json(pilot / APPROVAL_RECEIPT_FILENAME)
    if (
        approval_receipt.get("receipt_id") != APPROVAL_RECEIPT_ID
        or approval_receipt.get("status") != "valid"
        or approval_receipt.get("approved_commit") != ACCEPTED_SCRIPT_COMMIT
        or approval_receipt.get("approved_file_hashes") != approved_hashes
    ):
        raise ValueError("HUMAN_APPROVAL_RECEIPT_INVALID")
    lineage_hashes = {
        name: _sha256(pilot / name)
        for name in (APPROVAL_RECEIPT_FILENAME, *LINEAGE_LOCK_FILES)
    }
...
def _expected_contract(inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": (
            "new_banknote_yymm4_import_observation.expected_result_contract.v1"
        ),
        "status": "ready",
        ...
        "approval_lock": {
            ...
            "receipt_sha256": inputs["lineage_hashes"][APPROVAL_RECEIPT_FILENAME],
            "status": inputs["approval_receipt"]["status"],
            "approved_commit": ACCEPTED_SCRIPT_COMMIT,
        },
        "content_lineage_hashes": inputs["lineage_hashes"],
        "character_profile": {
        ...
        "operator_observation_fields": {
            ...
            "pronunciation_or_clipping_notes_by_cue": "required_may_be_empty",
...
3. pronunciation_notes: <success; empty allowed> | error: <failure only>
"""
...
def _manifest(
    ...
    return {
        ...
        "approved_script_identity": {
            ...
            "lineage_and_receipt_hashes": inputs["lineage_hashes"],
            "approval_status": inputs["approval_receipt"]["status"],
        },
        "character_profile": {
            "repo_relative_path": PROFILE_RELATIVE.as_posix(),
            "profile_id": PROFILE_ID,
        ...
        "manual_actions": [
            ...
            "YMM4に未保存の無関係な作業がないことを確認して新規空projectを開く",
            (
                "display済みCSVをimportし、mapping/error/update/"
                "character mismatchがなければdisplay済み.local.ymmpへ保存する"
            ),
            "9 cuesを一度previewして発音または明らかなclippingをcue番号でメモする",
            "renderせずYMM4を閉じ、terminalでCOLLECTとメモ1行を入力する",
        ],
        "return_item_count": 3,
        "return_items": [
            "success_or_failure",
            "operator_result_json_path",
            "pronunciation_notes_on_success_or_error_on_failure",
        ],
        "stop_conditions": [
        ...
        "collector_contract": {
            ...
            "mapping_dialog_status_required": True,
            "pronunciation_or_clipping_notes_recorded": True,
            "ImageItem_required": False,
            "independent_TextItem_required": False,
            "render_required": False,
        },
        ...
        "operator_messages": {
            ...
            "manual_sequence": (
                "新規空projectへimportして保存し、9 cuesを一度previewしてから、"
                "renderせずYMM4を安全に閉じてterminalへ戻ってください。"
            ),
            "collect_prompt": (
            ...
            "pronunciation_prompt": (
                "cue番号付きの発音または明らかなclippingメモを1行で入力。"
                "問題がなければ空Enter"
            ),
        },
        "prohibited_actions": [
...
def render_new_banknote_yymm4_operator_artifacts(
    ...
    preflight = {
        ...
        "checks": {
            ...
            "human_approval_receipt_valid": True,
            "content_lineage_complete": True,
            "manual_action_count_at_most_5": True,
        ...
        "approval_and_lineage_hashes": inputs["lineage_hashes"],
        "manual_action_count": 5,
        "return_item_count": 3,
        ...
        "runtime_command": (
            ...
            ".\\run_new_banknote_yymm4_batch.ps1 -PreflightOnly"
        ),
        "note": (
            "Worker-safe runtime preflight returns before executable resolution "
            "and does not inspect or launch YMM4."
        ),
    }
    return {
        **with_manifest,
...
def preflight_new_banknote_yymm4_operator_batch(
    ...
    preflight_parses_powershell = (
        ...
        and "collect_new_banknote_yymm4_result.ps1" in run_text
    )
    ...
    static_checks = {
        ...
        "collector_rechecks_full_preflight": (
            "new_banknote_yymm4_import_operator_batch preflight" in collect_text
        ),
        "preflight_returns_before_yymm4_inspection": (
            run_text.index("if ($PreflightOnly)")
            < run_text.index("$Ymm4 = Resolve-Ymm4Exe")
        ),
...
def collect_new_banknote_yymm4_import_result(
    *,
    ...
    not_before_utc: str,
    operator_confirmed_no_mapping_error: bool = False,
    ...
    expected_observation = (
        pilot / LOCAL_OUTPUT_DIRNAME / LOCAL_OBSERVATION_FILENAME
    ).resolve()
    observation_file = (
        Path(observation_path).resolve()
        if observation_path is not None
        else None
    )
    if observation_file is not None:
        if not _same_path(observation_file, expected_observation):
            raise ValueError("OPERATOR_OBSERVATION_TARGET_MUST_BE_EXACT_LOCAL_PATH")
        observation = _read_json(observation_file)
        observation_handoff = "explicit_utf8_json_file"
    else:
        observation = {
            "mapping_dialog_status": (
                "no_error_confirmed"
                if operator_confirmed_no_mapping_error
                else "not_confirmed"
            ),
            "pronunciation_or_clipping_notes": pronunciation_notes,
        }
        observation_handoff = "direct_python_api_compatibility"
    mapping_confirmed = (
        observation.get("mapping_dialog_status") == "no_error_confirmed"
    )
    observed_notes = observation.get("pronunciation_or_clipping_notes")
    notes_valid = isinstance(observed_notes, str)
    if not notes_valid:
        observed_notes = ""
    ...
    checks: dict[str, bool] = {
        "approval_and_content_lineage_lock_valid": True,
        ...
        "pronunciation_or_clipping_notes_valid": notes_valid,
        "explicit_observation_json_when_cli_used": (
            observation_file is None or observation_file.is_file()
        ),
    ...
    result = {
        ...
        "operator_observation": {
            ...
            "mapping_dialog_or_error_observed": (
                False if mapping_confirmed else None
            ),
            "pronunciation_or_clipping_notes": observed_notes,
            "handoff": observation_handoff,
            "evidence_grade": "observed",
...
def _parser() -> argparse.ArgumentParser:
    ...
    collect_parser.add_argument("--observation-json")
    ...
    collect_parser.add_argument("--pronunciation-notes", default="")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "collect":
        result = collect_new_banknote_yymm4_import_result(
            ...
            pronunciation_notes=args.pronunciation_notes,
