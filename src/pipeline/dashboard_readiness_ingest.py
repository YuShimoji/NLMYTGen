...
DEFAULT_ARTIFACT_ID = "dashboard_readiness_ingest_001"
...
def validate_dashboard_readiness_ingest_package(
    ...
    *,
    ...
    transcript_state = _row_state(rows, "transcript_substitution")
...
def _capability_rows(snapshot: dict[str, Any], output_root: Path, repo_root: Path) -> list[dict[str, Any]]:
    ...
    return [
        ...
        _row(
            ...
            "drop a real NotebookLM/human-reviewed transcript here or rerun with --transcript",
            repo_root,
            review_ready=False,
        ),
        _row(
...
def _readiness_summary_payload(
    *,
    ...
    return {
        ...
        "capability_rows": capability_rows,
        "next_action": (
            "Review dashboard_preview.md and readiness_summary.json, then supply a real transcript "
            "or rerun build-transcript-substitution with --transcript before YMM4 import preview work."
        ),
    }
    ...
    *,
    ...
    return {
        ...
        "route": [
            ...
            {"node": "content_spine", "state": _state_for(capability_rows, "content_spine")},
            {"node": "ir_bridge", "state": _state_for(capability_rows, "ir_bridge")},
            {"node": "transcript_substitution", "state": _state_for(capability_rows, "transcript_substitution")},
...
def _symbolic_visual_panel(capability_rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "bars": [
            _bar("content_spine", "Content spine", _state_for(capability_rows, "content_spine"), 5),
            _bar("ir_bridge", "IR/CSV bridge", _state_for(capability_rows, "ir_bridge"), 5),
            _bar(
                "transcript_substitution",
                "Transcript substitution",
                _state_for(capability_rows, "transcript_substitution"),
                4,
            ),
